...
if not DevAbs:get(Server.."IdBANDE") then 
...
os.execute('lua BANDE.lua') 
end ---ifBn
if Abs.Result.Info == 'Ok' then
io.write('\27[1;36mتم حفظ ايدي المطور الاساسي\n27[0;39;49m') 
DevAbs:set(Server.."IdBANDE",DevId) 
end ---ifok
else 
...
if not DevAbs:get(Server.."TokenBANDE") then 
...
DevAbs:set(Server.."TokenBANDE",TokenBot) 
end  
else 
...
Config = {
DevId = DevAbs:get(Server.."IdBANDE"),
TokenBot = DevAbs:get(Server.."TokenBANDE"),
BANDE = DevAbs:get(Server.."TokenBANDE"):match("(%d+)"),
...
#!/usr/bin/env bash
cd $HOME/BANDE
token="]]..DevAbs:get(Server.."TokenBANDE")..[["
while(true) do
rm -fr ../.telegram-cli
if [ ! -f ./tg ]; then
echo "┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉"
...
./tg -s ./BANDE.lua -p PROFILE --bot=$token
done
]])  
file:close()  
file = io.open("Run", "w")  
...
while(true) do
rm -fr ../.telegram-cli
screen -S BANDE -X kill
screen -S BANDE ./BANDE.sh
done
]]) 
file:close() 
...
local Load_BANDE = function() 
local f = io.open("./config.lua", "r") 
if not f then 
AutoSet() 
else 
f:close() 
DevAbs:del(Server.."IdBANDE");DevAbs:del(Server.."TokenBANDE")
end 
...
local Status = DevAbs:sismember(BANDE..'Abs:AbsSudo:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(BANDE..'Abs:SecondSudo:',msg.sender_user_id_) 
...
function Bot(msg) 
local var = false  
if msg.sender_user_id_ == tonumber(BANDE) then  
var = true  
end  
return var  
end 
...
local Status = DevAbs:sismember(BANDE..'Abs:SudoBot:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(BANDE..'Abs:AbsConstructor:'..msg.chat_id_,msg.sender_user_id_) 
if Status or AbsSudo(msg) or Sudo(msg) or SecondSudo(msg) or Bot(msg) then  
return true  
else  
return false  
...
----   Owner   ----
function Owner(msg) 
local Status = DevAbs:sismember(BANDE..'Abs:Owner:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(BANDE..'Abs:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(BANDE..'Abs:Constructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(BANDE..'Abs:Managers:'..msg.chat_id_,msg.sender_user_id_) 
...
----------  Admin  ---------
function Admin(msg) 
local Status = DevAbs:sismember(BANDE..'Abs:Admins:'..msg.chat_id_,msg.sender_user_id_) 
...
---------Vip Member---------
function VipMem(msg) 
local Status = DevAbs:sismember(BANDE..'Abs:VipMem:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(BANDE..'Abs:Cleaner:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(BANDE..'Abs:CleanerNum:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(BANDE..'Abs:CleanerMusic:'..msg.chat_id_,msg.sender_user_id_) 
if Status or SudoBot(msg) or Cleaner(msg) or AbsConstructor(msg) or AbsSudo(msg) or Sudo(msg) or SecondSudo(msg) or Bot(msg) then  
return true  
else  
return false  
...
if DevAbs:sismember(BANDE..'Abs:Ban:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(BANDE..'Abs:BanAll:', user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(BANDE..'Abs:Muted:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(BANDE..'Abs:MuteAll:', user_id) then
var = true
else
var = false
end
...
if plugin.BANDE and msg then
FilesText = plugin.BANDE(msg)
end
end
end
...
if File_Name:lower():match('(%d+)') ~= BANDE:lower() then 
...
local info_file = io.open('./'..BANDE..'.json', "r"):read('*a')
local JsonInfo = JSON.decode(info_file)
vardump(JsonInfo)
DevAbs:set(BANDE.."Abs:NameBot",JsonInfo.BotName) 
for IdGps,v in pairs(JsonInfo.GroupsList) do
DevAbs:sadd(BANDE.."Abs:Groups",IdGps) 
DevAbs:set(BANDE.."Abs:Lock:Bots"..IdGps,"del") DevAbs:hset(BANDE.."Abs:Spam:Group:User"..IdGps ,"Spam:User","keed") 
...
DevAbs:set(BANDE..Lock..IdGps,true)
end
if v.AbsConstructors then
for k,IdAbsConstructors in pairs(v.AbsConstructors) do
DevAbs:sadd(BANDE..'Abs:AbsConstructor:'..IdGps,IdAbsConstructors)  
print('تم رفع منشئين المجموعات')
end
end
if v.BasicConstructors then
for k,IdBasicConstructors in pairs(v.BasicConstructors) do
DevAbs:sadd(BANDE..'Abs:BasicConstructor:'..IdGps,IdBasicConstructors)  
print('تم رفع ( '..k..' ) منشئين اساسيين')
end
end
if v.Constructors then
for k,IdConstructors in pairs(v.Constructors) do
DevAbs:sadd(BANDE..'Abs:Constructor:'..IdGps,IdConstructors)  
print('تم رفع ( '..k..' ) منشئين')
end
end
if v.Managers then
for k,IdManagers in pairs(v.Managers) do
DevAbs:sadd(BANDE..'Abs:Managers:'..IdGps,IdManagers)  
...
vardump(IdAdmins)
DevAbs:sadd(BANDE..'Abs:Admins:'..IdGps,IdAdmins)  
print('تم رفع ( '..k..' ) ادمنيه')
end
end
if v.Vips then
for k,IdVips in pairs(v.Vips) do
DevAbs:sadd(BANDE..'Abs:VipMem:'..IdGps,IdVips)  
print('تم رفع ( '..k..' ) مميزين')
end
end
if v.LinkGroups then
if v.LinkGroups ~= "" then
DevAbs:set(BANDE.."Abs:Groups:Links"..IdGps,v.LinkGroups)   
print('( تم وضع روابط المجموعات )')
end
end
if v.Welcomes then
if v.Welcomes ~= "" then
DevAbs:set(BANDE.."Abs:Groups:Welcomes"..IdGps,v.Welcomes)   
...
var = true  
elseif tonumber(user_id) == tonumber(BANDE) then  
var = true
elseif DevAbs:sismember(BANDE..'Abs:AbsSudo:', user_id) then
var = true
elseif DevAbs:sismember(BANDE..'Abs:SecondSudo:', user_id) then
var = true  
elseif DevAbs:sismember(BANDE..'Abs:SudoBot:', user_id) then
var = true 
elseif DevAbs:sismember(BANDE..'Abs:AbsConstructor:'..chat_id, user_id) then
var = true
elseif DevAbs:sismember(BANDE..'Abs:Owner:'..chat_id, user_id) then
var = true
elseif DevAbs:sismember(BANDE..'Abs:BasicConstructor:'..chat_id, user_id) then
var = true
elseif DevAbs:sismember(BANDE..'Abs:Constructor:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(BANDE..'Abs:Managers:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(BANDE..'Abs:Admins:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(BANDE..'Abs:VipMem:'..chat_id, user_id) then  
var = true 
else  
var = false
end  
...
local Abs = (BANDE..'Abs:Filters:'..msg.chat_id_)
if Abs then
...
local UserName = (dp.username_ or "Dev-BANDE")
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
function Absmoned(chat_id, user_id, msg_id, text, offset, length) local tt = DevAbs:get(BANDE..'endmsg') or '' tdcli_function ({ ID = "SendMessage", chat_id_ = chat_id, reply_to_message_id_ = msg_id, disable_notification_ = 0, from_background_ = 1, reply_markup_ = nil, input_message_content_ = { ID = "InputMessageText", text_ = text..'\n\n'..tt, disable_web_page_preview_ = 1, clear_draft_ = 0, entities_ = {[0]={ ID="MessageEntityMentionName", offset_=offset, length_=length, user_id_=user_id }, }, }, }, dl_cb, nil) end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
function ChCheck(msg)
local var = true 
if DevAbs:get(BANDE.."Abs:ChId") then
local url , res = https.request('https://api.telegram.org/bot'..TokenBot..'/getchatmember?chat_id='..DevAbs:get(BANDE..'Abs:ChId')..'&user_id='..msg.sender_user_id_)
local data = json:decode(url)
if res ~= 200 or data.result.status == "left" or data.result.status == "kicked" then
var = false 
if DevAbs:get(BANDE..'Abs:ChText') then
local ChText = DevAbs:get(BANDE..'Abs:ChText')
send(msg.chat_id_,msg.id_,'['..ChText..']')
else
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChat?chat_id='..DevAbs:get(BANDE.."Abs:ChId"))
local GetInfo = JSON.decode(Check)
...
if DataText == '/delyes' and DevAbs:get(BANDE..'yes'..data.sender_user_id_) == 'delyes' then
DevAbs:del(BANDE..'yes'..data.sender_user_id_, 'delyes')
DevAbs:del(BANDE..'no'..data.sender_user_id_, 'delno')
...
if DataText == '/delno' and DevAbs:get(BANDE..'no'..data.sender_user_id_) == 'delno' then
DevAbs:del(BANDE..'yes'..data.sender_user_id_, 'delyes')
DevAbs:del(BANDE..'no'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "✯︙تم الغاء امر اطردني") 
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DataText == '/yesdel' and DevAbs:get(BANDE..'yesdel'..data.sender_user_id_) == 'delyes' then
DevAbs:del(BANDE..'yesdel'..data.sender_user_id_, 'delyes')
DevAbs:del(BANDE..'nodel'..data.sender_user_id_, 'delno')
if DevAbs:sismember(BANDE..'Abs:Constructor:'..data.chat_id_, data.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(BANDE..'Abs:Managers:'..data.chat_id_, data.sender_user_id_) then
Managers = 'المدراء • ' else Managers = '' end
if DevAbs:sismember(BANDE..'Abs:Admins:'..data.chat_id_, data.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevAbs:sismember(BANDE..'Abs:VipMem:'..data.chat_id_, data.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(BANDE..'Abs:Cleaner:'..data.chat_id_, data.sender_user_id_) then
cleaner = 'المنظفين • ' else cleaner = '' end
if DevAbs:sismember(BANDE..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
donky = 'المطايه • ' else donky = '' end
if DevAbs:sismember(BANDE..'Abs:Constructor:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(BANDE..'Abs:Managers:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(BANDE..'Abs:Admins:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(BANDE..'Abs:VipMem:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(BANDE..'Abs:Cleaner:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(BANDE..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
DevAbs:srem(BANDE..'Abs:Constructor:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(BANDE..'Abs:Managers:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(BANDE..'Abs:Admins:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(BANDE..'Abs:VipMem:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(BANDE..'Abs:Cleaner:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(BANDE..'User:Donky:'..data.chat_id_,data.sender_user_id_)
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE.."Abs:NewDev"..data.sender_user_id_) then
if DataText == '/setno' then
EditMsg(Chat_Id2, Msg_Id2, "✯︙تم الغاء امر تغير المطور الاساسي") 
DevAbs:del(BANDE.."Abs:NewDev"..data.sender_user_id_)
return false
end
if DataText == '/setyes' then
local NewDev = DevAbs:get(BANDE.."Abs:NewDev"..data.sender_user_id_)
...
if DataText == '/nodel' and DevAbs:get(BANDE..'nodel'..data.sender_user_id_) == 'delno' then
DevAbs:del(BANDE..'yesdel'..data.sender_user_id_, 'delyes')
DevAbs:del(BANDE..'nodel'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "✯︙تم الغاء امر نزلني") 
end
if DataText == '/YesRolet' and DevAbs:get(BANDE.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_) then
local List = DevAbs:smembers(BANDE..'Abs:ListRolet'..data.chat_id_) 
local UserName = List[math.random(#List)]
tdcli_function ({ID="SearchPublicChat",username_ = UserName},function(arg,dp) 
DevAbs:incrby(BANDE..'Abs:GamesNumber'..data.chat_id_..dp.id_, 5) 
end,nil) 
DevAbs:del(BANDE..'Abs:ListRolet'..data.chat_id_) 
DevAbs:del(BANDE.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
EditMsg(Chat_Id2, Msg_Id2, "✯︙*صاحب الحظ* ↫ ["..UserName.."]\n✯︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*")
end
if DataText == '/NoRolet' then
DevAbs:del(BANDE..'Abs:ListRolet'..data.chat_id_) 
DevAbs:del(BANDE.."Abs:NumRolet"..data.chat_id_..data.sender_user_id_) 
DevAbs:del(BANDE.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
...
if DevAbs:sismember(BANDE..'Abs:Tkeed:'..Chat_Id2, data.sender_user_id_) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..Chat_Id2.."&user_id="..data.sender_user_id_.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
DevAbs:srem(BANDE..'Abs:Tkeed:'..Chat_Id2, data.sender_user_id_)
...
DeleteMessage(Chat_Id2,{[0] = MsgId2})
https.request('https://ccccxcc.ml/BANDE/Yt.php?Put=Photo&TokenBot='..TokenBot..'&ChatId='..Chat_Id2..'&MsgId='..Msg_Id2..'&UserId='..data.sender_user_id_..'&Url='..Url..'&Name='..DirName)
...
https.request("https://api.telegram.org/bot"..TokenBot..'/answercallbackquery?callback_query_id='..data.id_..'&text='..URL.escape("✯ جار تحميل النتائج يرجى الانتظار .. .")..'&show_alert=true')
https.request('https://ccccxcc.ml/BANDE/Yt.php?Put=Video&TokenBot='..TokenBot..'&ChatId='..Chat_Id2..'&MsgId='..Msg_Id2..'&Url='..Url..'&Name='..DirName)
...
https.request("https://api.telegram.org/bot"..TokenBot..'/answercallbackquery?callback_query_id='..data.id_..'&text='..URL.escape("✯ جار تحميل النتائج يرجى الانتظار .. .")..'&show_alert=true')
https.request('https://ccccxcc.ml/BANDE/Yt.php?Put=Audio&TokenBot='..TokenBot..'&ChatId='..Chat_Id2..'&MsgId='..Msg_Id2..'&Url='..Url..'&Name='..DirName)
...
https.request("https://api.telegram.org/bot"..TokenBot..'/answercallbackquery?callback_query_id='..data.id_..'&text='..URL.escape("✯ جار تحميل النتائج يرجى الانتظار .. .")..'&show_alert=true')
DeleteMessage(Chat_Id2,{[0] = MsgId2})
https.request('https://ccccxcc.ml/BANDE/Yt.php?Put=Voice&TokenBot='..TokenBot..'&ChatId='..Chat_Id2..'&MsgId='..Msg_Id2..'&Url='..Url..'&Name='..DirName)
end
if DataText and DataText:match('/DelRed:'..tonumber(data.sender_user_id_)..'(.*)') then
local Abs = DataText:match('/DelRed:'..tonumber(data.sender_user_id_)..'(.*)')
EditMsg(Chat_Id2, Msg_Id2, "✯︙الكلمه ↫ "..Abs.." تم حذفها") 
DevAbs:del(BANDE..'Abs:Text:GpTexts'..Abs..data.chat_id_)
DevAbs:srem(BANDE..'Abs:Manager:GpRedod'..data.chat_id_,Abs)
end
if DataText and DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)') then
local Abs = DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)')
local List = DevAbs:smembers(BANDE..'Abs:Text:GpTexts'..Abs..data.chat_id_)
if DevAbs:get(BANDE..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "✯︙تم انهاء وحفظ ↫ "..#List.." من الردود المتعدده للامر ↫ "..Abs) 
DevAbs:del(BANDE..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
DevAbs:del(BANDE..'Abs:Text:GpTexts'..Abs..data.chat_id_)
DevAbs:del(BANDE..'Abs:Add:GpTexts'..data.sender_user_id_..data.chat_id_)
DevAbs:srem(BANDE..'Abs:Manager:GpRedod'..data.chat_id_,Abs)
else
EditMsg(Chat_Id2, Msg_Id2, "✯︙عذرا صلاحية الامر منتهيه !") 
end
...
DevAbs:del(BANDE..'Abs:Text:KtTexts'..Abs..data.chat_id_)
DevAbs:srem(BANDE..'Abs:Sudo:Kt'..data.chat_id_,Abs)
end
...
local List = DevAbs:smembers(BANDE..'Abs:Text:KtTexts'..Abs..data.chat_id_)
if DevAbs:get(BANDE..'Abs:Add:Kt'..data.sender_user_id_..data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "✯︙تم انهاء وحفظ ↫ "..#List.." من الاسئله للامر ↫ "..Abs) 
DevAbs:del(BANDE..'Abs:Add:Kt'..data.sender_user_id_..data.chat_id_)
else
EditMsg(Chat_Id2, Msg_Id2, "✯︙عذرا صلاحية الامر منتهيه !") 
end
end
...
DevAbs:del(BANDE..'Abs:Add:Kt'..data.sender_user_id_..data.chat_id_)
DevAbs:del(BANDE..'Abs:Text:KtTexts'..Abs..data.chat_id_)
DevAbs:del(BANDE..'Abs:Add:KtTexts'..data.sender_user_id_..data.chat_id_)
DevAbs:srem(BANDE..'Abs:Sudo:Kt'..data.chat_id_,Abs)
else
EditMsg(Chat_Id2, Msg_Id2, "✯︙عذرا صلاحية الامر منتهيه !") 
end
...
return https.request("https://api.telegram.org/bot"..TokenBot..'/sendAudio?chat_id=' .. data.chat_id_ .. '&audio=https://t.me/BANDEMp3/'..Abs..'&caption=' .. URL.escape(Text).."&reply_to_message_id="..Msg_Id2.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard)) 
else
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
DevAbs:sadd(BANDE..'Abs:VipMem:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(BANDE..'Abs:Cleaner:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(BANDE..'Abs:Admins:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(BANDE..'Abs:Managers:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(BANDE..'Abs:Constructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(BANDE..'Abs:BasicConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(BANDE..'Abs:AbsConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(BANDE..'Abs:SudoBot:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(BANDE..'Abs:SecondSudo:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:sadd(BANDE..'Abs:AbsSudo:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:VipMem:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:Cleaner:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:Admins:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:Managers:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:Constructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:BasicConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:AbsConstructor:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:SudoBot:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:SecondSudo:'..data.chat_id_,dp.id_)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:AbsSudo:'..data.chat_id_,dp.id_)
...
DevAbs:sadd(BANDE..'Abs:Ban:'..data.chat_id_, dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'✯︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'WaTaNTeaM')..')\n✯︙تم حظره')
end
end
end,nil)
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:Ban:'..data.chat_id_,dp.id_)
...
if DevAbs:sismember(BANDE..'Abs:Muted:'..data.chat_id_, dp.id_) then
EditMsg(Chat_Id2, Msg_Id2, "✯︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(BANDE..'Abs:Muted:'..data.chat_id_, dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'✯︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'WaTaNTeaM')..')\n✯︙تم كتمه')
end
end
end
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:Muted:'..data.chat_id_, dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'✯︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'WaTaNTeaM')..')\n✯︙تم الغاء كتمه')
end,nil)
end
...
tdcli_function ({ID = "GetUser",user_id_ = AbsId},function(arg,dp) 
DevAbs:srem(BANDE..'Abs:BanAll:', dp.id_)
DevAbs:srem(BANDE..'Abs:MuteAll:', dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'✯︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'WaTaNTeaM')..')\n✯︙تم الغاء العام')
end,nil)
end
...
DevAbs:sadd(BANDE..'Abs:Tkeed:'..data.chat_id_, dp.id_)
...
DevAbs:srem(BANDE..'Abs:Tkeed:'..data.chat_id_, dp.id_)
EditMsg(Chat_Id2, Msg_Id2,'✯︙العضو ↫ ['..dp.first_name_..'](t.me/'..(dp.username_ or 'WaTaNTeaM')..')\n✯︙تم الغاء تقيده')
end,nil)
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
elseif Constructor(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..Abs},{text="تنزيل ادمن",callback_data=data.sender_user_id_..":RemAdmin:"..Abs}},{{text="تنزيل منظف",callback_data=data.sender_user_id_..":RemCleaner:"..Abs},{text="تنزيل مدير",callback_data=data.sender_user_id_..":RemManager:"..Abs}},{{text = '⌁ BANDE TeAM.',url="t.me/WaTaNTeaM"}}}
elseif Manager(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=data.sender_user_id_..":RemMem:"..Abs},{text="تنزيل ادمن",callback_data=data.sender_user_id_..":RemAdmin:"..Abs}}}
elseif Admin(data) then
...
local Del = DevAbs:get(BANDE..'Abs:Del')
local Text = [[
...
DevAbs:del(BANDE..'Abs:SecondSudo:')
...
DevAbs:del(BANDE..'Abs:Owner:'..data.chat_id_)
...
DevAbs:del(BANDE..'Abs:UsersMsgs'..data.chat_id_..':'..data.sender_user_id_)
...
DevAbs:del(BANDE..'Abs:Constructor:'..data.chat_id_)
...
DevAbs:del(BANDE..'Abs:Managers:'..data.chat_id_)
Text = "*✯︙تم حذف المدراء*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• القائمه الرئيسيه •",callback_data="/DelList:"..data.sender_user_id_}}}
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevAbs:del(BANDE..'Abs:Admins:'..data.chat_id_)
...
DevAbs:del(BANDE..'Abs:VipMem:'..data.chat_id_)
...
DevAbs:del(BANDE..'Abs:Cleaner:'..data.chat_id_)
...
DevAbs:del(BANDE..'Abs:Muted:'..data.chat_id_)
...
local List = DevAbs:smembers(BANDE..'Abs:Tkeed:'..data.chat_id_)
for k,v in pairs(List) do   
...
DevAbs:srem(BANDE..'Abs:Tkeed:'..data.chat_id_, v)
end
...
DevAbs:del(BANDE..'Abs:Ban:'..data.chat_id_)
...
DevAbs:del(BANDE..'Abs:BanAll:')
DevAbs:del(BANDE..'Abs:MuteAll:')
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DataText and DataText:match('/DelGpRed:'..tonumber(data.sender_user_id_)..'(.*)') and not DevAbs:get(BANDE..'Abs:Lock:GpRd'..data.chat_id_) then
local Abs = DataText:match('/DelGpRed:'..tonumber(data.sender_user_id_)..'(.*)')
local redod = DevAbs:smembers(BANDE..'Abs:Manager:GpRed'..data.chat_id_)
if #redod == 0 then
Text = "*✯︙لا توجد ردود مضافه*"
else
for k,v in pairs(redod) do
DevAbs:del(BANDE..'Abs:Gif:GpRed'..v..data.chat_id_)
DevAbs:del(BANDE..'Abs:Voice:GpRed'..v..data.chat_id_)
DevAbs:del(BANDE..'Abs:Audio:GpRed'..v..data.chat_id_)
DevAbs:del(BANDE..'Abs:Photo:GpRed'..v..data.chat_id_)
DevAbs:del(BANDE..'Abs:Stecker:GpRed'..v..data.chat_id_)
DevAbs:del(BANDE..'Abs:Video:GpRed'..v..data.chat_id_)
DevAbs:del(BANDE..'Abs:File:GpRed'..v..data.chat_id_)
DevAbs:del(BANDE..'Abs:Text:GpRed'..v..data.chat_id_)
DevAbs:del(BANDE..'Abs:Manager:GpRed'..data.chat_id_)
end
...
if DataText and DataText:match('/DelGpRedAll:'..tonumber(data.sender_user_id_)..'(.*)') and not DevAbs:get(BANDE..'Abs:Lock:GpRd'..data.chat_id_) then
local Abs = DataText:match('/DelGpRedAll:'..tonumber(data.sender_user_id_)..'(.*)')
local redod = DevAbs:smembers(BANDE.."Abs:Sudo:AllRed")
if #redod == 0 then
Text = "*✯︙لا توجد ردود مضافه*"
else
for k,v in pairs(redod) do
DevAbs:del(BANDE.."Abs:Add:AllRed"..v)
DevAbs:del(BANDE.."Abs:Gif:AllRed"..v)
DevAbs:del(BANDE.."Abs:Voice:AllRed"..v)
DevAbs:del(BANDE.."Abs:Audio:AllRed"..v)
DevAbs:del(BANDE.."Abs:Photo:AllRed"..v)
DevAbs:del(BANDE.."Abs:Stecker:AllRed"..v)
DevAbs:del(BANDE.."Abs:Video:AllRed"..v)
DevAbs:del(BANDE.."Abs:File:AllRed"..v)
DevAbs:del(BANDE.."Abs:Text:AllRed"..v)
DevAbs:del(BANDE.."Abs:Sudo:AllRed")
end
...
local List = DevAbs:smembers(BANDE.."List:Cmd:Group:New"..data.chat_id_)
for k,v in pairs(List) do
DevAbs:del(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":"..v)
DevAbs:del(BANDE.."List:Cmd:Group:New"..data.chat_id_)
end
...
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":ا","ايدي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"ا")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":م","رفع مميز")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"م")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":اد","رفع ادمن")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"اد")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":مد","رفع مدير")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"مد")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":من","رفع منشئ")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"من")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":اس","رفع منشئ اساسي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"اس")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":مط","رفع مطور")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"مط")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":ثانوي","رفع مطور ثانوي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"ثانوي")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":تك","تنزيل الكل")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"تك")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":تعط","تعطيل الايدي بالصوره")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"تعط")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":تفع","تفعيل الايدي بالصوره")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"تفع")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":ر","الرابط")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"ر")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":رر","ردود المدير")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"رر")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":،،","مسح المكتومين")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"،،")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":رد","اضف رد")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"رد")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":غ","غنيلي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"غ")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..data.chat_id_..":#","مسح قائمه العام")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..data.chat_id_,"#")
...
DevAbs:sadd(BANDE..'Abs:Admins:'..data.chat_id_, admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(BANDE..'Abs:Admins:'..data.chat_id_, admins[i].user_id_)
end
end,nil)
else
DevAbs:sadd(BANDE..'Abs:Admins:'..data.chat_id_, admins[i].user_id_)
...
DevAbs:sadd(BANDE.."Abs:BasicConstructor:"..data.chat_id_,admins[i].user_id_)
DevAbs:sadd(BANDE.."Abs:AbsConstructor:"..data.chat_id_,admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(BANDE.."Abs:BasicConstructor:"..data.chat_id_,admins[i].user_id_)
DevAbs:srem(BANDE.."Abs:AbsConstructor:"..data.chat_id_,admins[i].user_id_)
end
end,nil)  
end 
end
end,nil)
if DevAbs:sismember(BANDE..'Abs:Groups',data.chat_id_) then
...
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
DevAbs:sadd(BANDE.."Abs:Groups",data.chat_id_)
if not DevAbs:get(BANDE..'Abs:SudosGp'..data.sender_user_id_..data.chat_id_) and not SecondSudo(data) then 
DevAbs:incrby(BANDE..'Abs:Sudos'..data.sender_user_id_,1)
DevAbs:set(BANDE..'Abs:SudosGp'..data.sender_user_id_..data.chat_id_,"Abs")
end
...
DevAbs:set(BANDE.."Abs:Groups:Links"..data.chat_id_,LinkGroup) 
...
tdcli_function ({ID = "GetUser",user_id_ = data.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=data.chat_id_},function(arg,dp) 
if not DevAbs:sismember(BANDE..'Abs:Groups',data.chat_id_) then
...
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
DevAbs:srem(BANDE.."Abs:Groups",data.chat_id_)
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
local Name1 = Name1:gsub("`","") 
...
DevAbs:sadd(BANDE..'Abs:BasicConstructor:'..data.chat_id_,Manager_id)  
DevAbs:sadd(BANDE..'Abs:AbsConstructor:'..data.chat_id_,Manager_id)   
end  
end  
if num == 0 then
...
local DelSudoRep = DevAbs:get(BANDE..'DelSudoRep')
DevAbs:del(BANDE.."Abs:Sudo:AllRed",DelSudoRep)
DevAbs:del(BANDE.."Abs:Add:AllText"..data.sender_user_id_)
DevAbs:del(BANDE.."Abs:Add:AllRed"..data.sender_user_id_)
DevAbs:del(BANDE.."DelSudoRep")
...
local DelManagerRep = DevAbs:get(BANDE..'DelManagerRep'..data.chat_id_)
DevAbs:srem(BANDE..'Abs:Manager:GpRed'..data.chat_id_,DelManagerRep)
DevAbs:del(BANDE..'Abs:Add:GpText'..data.sender_user_id_..data.chat_id_)
DevAbs:del(BANDE..'Abs:Add:GpRed'..data.sender_user_id_..data.chat_id_)
DevAbs:del(BANDE..'DelManagerRep'..data.chat_id_)
Text = "✯︙تم الغاء حفظ الرد"
keyboard = {} 
keyboard.inline_keyboard = {{{text = '✯ WaTaN TeaM .',url="t.me/WaTaNTeaM"}}}
https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local Cmd = DevAbs:get(BANDE..'Abs:Cmd')
local Text = [[
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local Msg = DevAbs:get(BANDE..'Abs:Msg')
local Text = [[
...
DevAbs:del(BANDE..'Abs:EditMsg'..data.chat_id_..data.sender_user_id_)
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local Filter = DevAbs:get(BANDE..'Abs:Filter')
local Text = [[
...
DevAbs:del(BANDE.."Abs:FilterAnimation"..data.chat_id_)
...
DevAbs:del(BANDE.."Abs:FilterSteckr"..data.chat_id_)
...
DevAbs:del(BANDE.."Abs:FilterPhoto"..data.chat_id_)
...
DevAbs:del(BANDE..'Abs:Filters:'..data.chat_id_)
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local locklist = DevAbs:get(BANDE..'Abs:locklist')
local Text = [[
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local LockBotList = DevAbs:get(BANDE..'Abs:LockBotList')
local Text = [[
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local LockSpamList = DevAbs:get(BANDE..'Abs:LockSpamList')
local Text = [[
...
if not DevAbs:get(BANDE..'Abs:Lock:Join'..data.chat_id_) then
DevAbs:set(BANDE..'Abs:Lock:Join'..data.chat_id_,true)
...
if DevAbs:get(BANDE..'Abs:Lock:Join'..data.chat_id_) then
DevAbs:del(BANDE..'Abs:Lock:Join'..data.chat_id_)
...
if not DevAbs:get(BANDE..'Abs:Lock:Text'..data.chat_id_) then
DevAbs:set(BANDE..'Abs:Lock:Text'..data.chat_id_,true)
...
if DevAbs:get(BANDE..'Abs:Lock:Text'..data.chat_id_) then
DevAbs:del(BANDE..'Abs:Lock:Text'..data.chat_id_)
...
if not DevAbs:get(BANDE..'Abs:Lock:TagServr'..data.chat_id_) then
DevAbs:set(BANDE..'Abs:Lock:TagServr'..data.chat_id_,true)
...
if DevAbs:get(BANDE..'Abs:Lock:TagServr'..data.chat_id_) then
DevAbs:del(BANDE..'Abs:Lock:TagServr'..data.chat_id_) 
...
if not DevAbs:get(BANDE..'Abs:Lock:EditMsgs'..data.chat_id_) then
DevAbs:set(BANDE..'Abs:Lock:EditMsgs'..data.chat_id_,true)
...
if DevAbs:get(BANDE..'Abs:Lock:EditMsgs'..data.chat_id_) then
DevAbs:del(BANDE..'Abs:Lock:EditMsgs'..data.chat_id_)
...
if not DevAbs:get(BANDE..'Abs:Lock:Stickers'..data.chat_id_) then
DevAbs:set(BANDE..'Abs:Lock:Stickers'..data.chat_id_,true)
...
if DevAbs:get(BANDE..'Abs:Lock:Stickers'..data.chat_id_) then
DevAbs:del(BANDE..'Abs:Lock:Stickers'..data.chat_id_)
...
if not DevAbs:get(BANDE..'Abs:Lock:Gifs'..data.chat_id_) then
DevAbs:set(BANDE..'Abs:Lock:Gifs'..data.chat_id_,true)
...
if DevAbs:get(BANDE..'Abs:Lock:Gifs'..data.chat_id_) then
DevAbs:del(BANDE..'Abs:Lock:Gifs'..data.chat_id_)
...
if not DevAbs:get(BANDE..'Abs:Lock:Photo'..data.chat_id_) then
DevAbs:set(BANDE..'Abs:Lock:Photo'..data.chat_id_,true) 
...
if DevAbs:get(BANDE..'Abs:Lock:Photo'..data.chat_id_) then
DevAbs:del(BANDE..'Abs:Lock:Photo'..data.chat_id_)
...
if not DevAbs:get(BANDE..'Abs:Lock:Videos'..data.chat_id_) then
DevAbs:set(BANDE..'Abs:Lock:Videos'..data.chat_id_,true)
...
if DevAbs:get(BANDE..'Abs:Lock:Videos'..data.chat_id_) then
DevAbs:del(BANDE..'Abs:Lock:Videos'..data.chat_id_)
...
if not DevAbs:get(BANDE..'Abs:Lock:Forwards'..data.chat_id_) then
DevAbs:set(BANDE..'Abs:Lock:Forwards'..data.chat_id_,true)
...
if DevAbs:get(BANDE..'Abs:Lock:Forwards'..data.chat_id_) then
DevAbs:del(BANDE..'Abs:Lock:Forwards'..data.chat_id_) 
...
DevAbs:set(BANDE.."Abs:Lock:Bots"..data.chat_id_,"kick")  
...
DevAbs:set(BANDE.."Abs:Lock:Bots"..data.chat_id_,"ked")   
...
DevAbs:set(BANDE.."Abs:Lock:Bots"..data.chat_id_,"del")  
...
DevAbs:del(BANDE.."Abs:Lock:Bots"..data.chat_id_) 
...
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..data.chat_id_ ,"Spam:User","kick")  
...
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..data.chat_id_ ,"Spam:User","keed")  
...
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..data.chat_id_ ,"Spam:User","del")  
...
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..data.chat_id_ ,"Spam:User","mute")  
...
DevAbs:hdel(BANDE.."Abs:Spam:Group:User"..data.chat_id_ ,"Spam:User")  
...
if not DevAbs:get(BANDE..'Abs:Lock:Gamesinline'..data.chat_id_) then
Text =[[
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local Source = DevAbs:get(BANDE..'Abs:Source')
local Text = [[
...
os.execute('rm -rf BANDE.lua') 
os.execute('wget https://raw.githubusercontent.com/Dev-BANDE/BANDE/main/BANDE.lua') 
dofile('BANDE.lua') 
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local Media = DevAbs:get(BANDE..'Abs:Media')
local Text = [[
...
local List = DevAbs:smembers(BANDE.."Abs:cleaner"..data.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
local Message = v
...
local List = DevAbs:smembers(BANDE.."Abs:cleanermusic"..data.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
local Message = v
...
DevAbs:del(BANDE.."Abs:cleanermusic"..data.chat_id_)
else
...
if DevAbs:get(BANDE.."Abs:Left:Bot"..BANDE) then
...
if not DevAbs:get(BANDE.."Abs:Lock:GpLinksinline"..data.chat_id_) then 
tdcli_function({ID ="GetChat",chat_id_=data.chat_id_},function(arg,ta) 
local linkgpp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..data.chat_id_)) or DevAbs:get(BANDE.."Private:Group:Link"..data.chat_id_) 
...
if not DevAbs:get(BANDE..'Abs:Lock:GpLinks'..data.chat_id_) then 
if DevAbs:get(BANDE.."Abs:Groups:Links"..data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "✯︙Group Link ↬ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"..DevAbs:get(BANDE.."Abs:Groups:Links"..data.chat_id_), 1, "html")
else 
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local order = DevAbs:get(BANDE..'Abs:order')
local Text = [[
...
DevAbs:set(BANDE..'Abs:Lock:Id:Photo'..data.chat_id_,true)
...
DevAbs:del(BANDE..'Abs:Lock:Id:Photo'..data.chat_id_)
...
DevAbs:set(BANDE..'Abs:Lock:Id'..data.chat_id_,true)
...
DevAbs:del(BANDE..'Abs:Lock:Id'..data.chat_id_)
...
DevAbs:set(BANDE.."Abs:Lock:GpLinks"..data.chat_id_,"ok")
...
DevAbs:del(BANDE.."Abs:Lock:GpLinks"..data.chat_id_)
...
DevAbs:set(BANDE.."Abs:Lock:GpLinksinline"..data.chat_id_,"ok")
...
DevAbs:del(BANDE.."Abs:Lock:GpLinksinline"..data.chat_id_)
...
DevAbs:del(BANDE.."Abs:Lock:Welcome"..data.chat_id_)
...
DevAbs:set(BANDE.."Abs:Lock:Welcome"..data.chat_id_,true)
...
DevAbs:set(BANDE..'Abs:Lock:AllRed'..data.chat_id_,true)
...
DevAbs:del(BANDE..'Abs:Lock:AllRed'..data.chat_id_)
...
DevAbs:set(BANDE..'Abs:Lock:GpRed'..data.chat_id_,true)
...
DevAbs:del(BANDE..'Abs:Lock:GpRed'..data.chat_id_)
...
DevAbs:set(BANDE.."Abs:Del:Me"..data.chat_id_, true)
Text = "*✯︙تم تعطيل نزلني*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevAbs:del(BANDE.."Abs:Del:Me"..data.chat_id_)
Text = "*✯︙تم تفعيل نزلني*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevAbs:set(BANDE.."Abs:Kick:Me"..data.chat_id_, true)
...
DevAbs:del(BANDE.."Abs:Kick:Me"..data.chat_id_)
...
DevAbs:set(BANDE.."Abs:Lock:KickBan"..data.chat_id_,"true")
Text = "*✯︙تم تعطيل الحظر*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevAbs:del(BANDE.."Abs:Lock:KickBan"..data.chat_id_)
Text = "*✯︙تم تفعيل الحظر*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevAbs:set(BANDE.."Abs:Lock:ProSet"..data.chat_id_,"true")
Text = "*✯︙تم تعطيل الرفع*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevAbs:del(BANDE.."Abs:Lock:ProSet"..data.chat_id_)
Text = "*✯︙تم تفعيل الرفع*"
keyboard = {} 
keyboard.inline_keyboard = {{{text="• اخفاء الكليشه •",callback_data="/HideHelpList:"..data.sender_user_id_},{text="• رجوع •",callback_data="/lockorder:"..data.sender_user_id_}}}
return https.request("https://api.telegram.org/bot"..TokenBot..'/editMessageText?chat_id='..Chat_Id2..'&message_id='..Msg_Id2..'&text=' .. URL.escape(Text).."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
...
DevAbs:set(BANDE..'Abs:Lock:Games'..data.chat_id_,true)  
...
DevAbs:del(BANDE..'Abs:Lock:Games'..data.chat_id_) 
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local Help = DevAbs:get(BANDE..'Abs:Help')
local Text = [[
...
local Help = DevAbs:get(BANDE..'Abs:Help1')
local Text = [[
...
local Help = DevAbs:get(BANDE..'Abs:Help2')
local Text = [[
✯︙اوامر الادمنيه ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✯︙الاعدادت
...
local Help = DevAbs:get(BANDE..'Abs:Help3')
local Text = [[
✯︙اوامر المدراء ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✯︙فحص البوت
...
local Help = DevAbs:get(BANDE..'Abs:Help4')
local Text = [[
✯︙اوامر المنشئين ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✯︙تنزيل الكل
...
local Help = DevAbs:get(BANDE..'Abs:Help5')
local Text = [[
✯︙اوامر المطورين ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✯︙الكروبات
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local Help = DevAbs:get(BANDE..'Abs:Help6')
local Text = [[
✯︙اوامر الاعضاء ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
if text and DevAbs:get(BANDE.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
local NewCmmd = DevAbs:get(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
if NewCmmd then
DevAbs:del(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
DevAbs:del(BANDE.."Set:Cmd:Group:New"..msg.chat_id_)
DevAbs:srem(BANDE.."List:Cmd:Group:New"..msg.chat_id_,text)
...
DevAbs:del(BANDE.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
if text and text:match('^'..(DevAbs:get(BANDE..'Abs:NameBot') or "وطن")..' ') then
data.message_.content_.text_ = data.message_.content_.text_:gsub('^'..(DevAbs:get(BANDE..'Abs:NameBot') or "وطن")..' ','')
end
if data.message_.content_.text_ then
local NewCmmd = DevAbs:get(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":"..data.message_.content_.text_)
if NewCmmd then
data.message_.content_.text_ = (NewCmmd or data.message_.content_.text_)
end
end
if text and DevAbs:get(BANDE.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
DevAbs:set(BANDE.."Set:Cmd:Group:New"..msg.chat_id_,text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙ارسل الامر الجديد", 1, 'html')
DevAbs:del(BANDE.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_)
DevAbs:set(BANDE.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_,"true1") 
return false
end
if text and DevAbs:get(BANDE.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_) == "true1" then
local NewCmd = DevAbs:get(BANDE.."Set:Cmd:Group:New"..msg.chat_id_)
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text,NewCmd)
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ الامر", 1, 'html')
DevAbs:del(BANDE.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
if Constructor(msg) then
if text == "الاوامر المضافه" and ChCheck(msg) then
local List = DevAbs:smembers(BANDE.."List:Cmd:Group:New"..msg.chat_id_.."") 
t = "✯︙قائمة الاوامر المضافه ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
Cmds = DevAbs:get(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
if Cmds then 
t = t..k.."~ ("..v..") • {"..Cmds.."}\n"
else
t = t..k.."~ ("..v..") \n"
...
local List = DevAbs:smembers(BANDE.."List:Cmd:Group:New"..msg.chat_id_)
for k,v in pairs(List) do
DevAbs:del(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
DevAbs:del(BANDE.."List:Cmd:Group:New"..msg.chat_id_)
end
...
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":ا","ايدي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"ا")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":م","رفع مميز")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"م")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":اد","رفع ادمن")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"اد")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":مد","رفع مدير")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"مد")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":من","رفع منشئ")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"من")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":اس","رفع منشئ اساسي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"اس")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":مط","رفع مطور")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"مط")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":ثانوي","رفع مطور ثانوي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"ثانوي")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":تك","تنزيل الكل")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"تك")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":تعط","تعطيل الايدي بالصوره")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"تعط")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":تفع","تفعيل الايدي بالصوره")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"تفع")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":ر","الرابط")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"ر")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":رر","ردود المدير")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"رر")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":،،","مسح المكتومين")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"،،")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":رد","اضف رد")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"رد")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":سح","مسح سحكاتي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"سح")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":رس","مسح رسائلي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"رس")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":غ","غنيلي")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"غ")
DevAbs:set(BANDE.."Set:Cmd:Group:New1"..msg.chat_id_..":#","مسح قائمه العام")
DevAbs:sadd(BANDE.."List:Cmd:Group:New"..msg.chat_id_,"#")
send(msg.chat_id_, msg.id_,"✯︙تم ترتيب الاوامر بالشكل التالي ~\n✯︙ايدي - ا .\n✯︙رفع مميز - م .\n✯︙رفع ادمن - اد .\n✯︙رفع مدير - مد . \n✯︙رفع منشى - من . \n✯︙رفع منشئ الاساسي - اس  .\n✯︙رفع مطور - مط .\n✯︙رفع مطور ثانوي - ثانوي .\n✯︙تنزيل الكل - تك .\n✯︙تعطيل الايدي بالصوره - تعط .\n✯︙تفعيل الايدي بالصوره - تفع .\n✯︙الرابط - ر .\n✯︙ردود المدير - رر .\n✯︙مسح المكتومين - ،، .\n✯︙اضف رد - رد .\n✯︙مسح سحكاتي - سح .\n✯︙مسح رسائلي - رس .\n✯︙غنيلي - غ .\n✯︙مسح قائمه العام - # .")  
end
if text == "اضف امر" and ChCheck(msg) or text == "اضافة امر" and ChCheck(msg) or text == "اضافه امر" and ChCheck(msg) then
DevAbs:set(BANDE.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
DevAbs:set(BANDE.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
local List = DevAbs:smembers(BANDE.."Coomds"..msg.chat_id_)
if #List == 0 then
...
for k,v in pairs(List) do
var = DevAbs:get(BANDE.."Comd:New:rt:Abs:"..v..msg.chat_id_)
if var then
t = t..k.."~ "..v.." • ("..var..")\n"
else
t = t..k.."~ "..v.."\n"
...
local List = DevAbs:smembers(BANDE.."Coomds"..msg.chat_id_)
for k,v in pairs(List) do
DevAbs:del(BANDE.."Comd:New:rt:Abs:"..v..msg.chat_id_)
DevAbs:del(BANDE.."Coomds"..msg.chat_id_)
end
...
DevAbs:set(BANDE.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_,ComdNew)  
DevAbs:sadd(BANDE.."Coomds"..msg.chat_id_,ComdNew)  
DevAbs:setex(BANDE.."Comd:New"..msg.chat_id_..msg.sender_user_id_,200,true)  
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙ارسل نوع الصلاحيه \n{ عضو • مميز  • ادمن  • مدير }\n✯︙ارسل الغاء لالغاء الامر ", 1, 'html')
end
if text and text:match("^حذف صلاحيه (.*)$") and ChCheck(msg) or text and text:match("^مسح صلاحيه (.*)$") and ChCheck(msg) then 
ComdNew = text:match("^حذف صلاحيه (.*)$") or text:match("^مسح صلاحيه (.*)$")
DevAbs:del(BANDE.."Comd:New:rt:Abs:"..ComdNew..msg.chat_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حذف الصلاحيه", 1, 'html')
end
if DevAbs:get(BANDE.."Comd:New"..msg.chat_id_..msg.sender_user_id_) then 
if text and text:match("^↫ الغاء ✯$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء الامر", 1, 'html')
DevAbs:del(BANDE.."Comd:New"..msg.chat_id_..msg.sender_user_id_) 
return false  
end 
if text == "مدير" then
...
local textn = DevAbs:get(BANDE.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_)  
DevAbs:set(BANDE.."Comd:New:rt:Abs:"..textn..msg.chat_id_,text)
...
if DevAbs:sismember(BANDE.."Coomds"..msg.chat_id_,DEV_Abs) then
function by_reply(extra, result, success)   
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local mrAbs = DevAbs:get(BANDE.."Comd:New:rt:Abs:"..DEV_Abs..msg.chat_id_)
...
DevAbs:set(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_Abs) 
DevAbs:sadd(BANDE..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
elseif mrAbs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'WaTaNTeaM')..')'..' ❩\n✯︙تم رفعه ❨ '..DEV_Abs..' ❩ بنجاح', 1, 'md')
DevAbs:set(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_Abs)
DevAbs:sadd(BANDE..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
elseif mrAbs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'WaTaNTeaM')..')'..' ❩\n✯︙تم رفعه ❨ '..DEV_Abs..' ❩ بنجاح', 1, 'md')
DevAbs:set(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_,DEV_Abs)  
DevAbs:sadd(BANDE..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:srem(BANDE..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
DevAbs:del(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrAbs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'WaTaNTeaM')..')'..' ❩\n✯︙تم تنزيله ❨ '..DEV_Abs..' ❩ بنجاح', 1, 'md')
DevAbs:srem(BANDE..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
DevAbs:del(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrAbs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'WaTaNTeaM')..')'..' ❩\n✯︙تم تنزيله ❨ '..DEV_Abs..' ❩ بنجاح', 1, 'md')
DevAbs:srem(BANDE..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
DevAbs:del(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.sender_user_id_)
elseif mrAbs == "عضو" then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..data.first_name_..'](t.me/'..(data.username_ or 'WaTaNTeaM')..')'..' ❩\n✯︙تم تنزيله ❨ '..DEV_Abs..' ❩ بنجاح', 1, 'md')
end
...
if DevAbs:sismember(BANDE.."Coomds"..msg.chat_id_,text1[2]) then
function py_username(extra, result, success)   
if result.id_ then
local mrAbs = DevAbs:get(BANDE.."Comd:New:rt:Abs:"..text1[2]..msg.chat_id_)
if mrAbs == "مميز" and VipMem(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'WaTaNTeaM')..')'..' ❩\n✯︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:sadd(BANDE..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:set(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrAbs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'WaTaNTeaM')..')'..' ❩\n✯︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:sadd(BANDE..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:set(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrAbs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'WaTaNTeaM')..')'..' ❩\n✯︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:sadd(BANDE..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:set(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.id_,text1[2])
elseif mrAbs == "عضو" then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'WaTaNTeaM')..')'..' ❩\n✯︙تم رفعه ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
end
...
DevAbs:srem(BANDE..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:del(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrAbs == "ادمن" and Admin(msg) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'WaTaNTeaM')..')'..' ❩\n✯︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:srem(BANDE..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:del(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrAbs == "مدير" and Manager(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'WaTaNTeaM')..')'..' ❩\n✯︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
DevAbs:srem(BANDE..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:del(BANDE.."Comd:New:rt:User:"..msg.chat_id_..result.id_)
elseif mrAbs == "عضو" then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو ↫ ❨ ['..result.title_..'](t.me/'..(text1[3] or 'WaTaNTeaM')..')'..' ❩\n✯︙تم تنزيله ❨ '..text1[2]..' ❩ بنجاح', 1, 'md')
end
...
DevAbs:incr(BANDE..'Abs:UsersMsgs'..BANDE..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:incr(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:incr(BANDE..'Abs:MsgNumberDay'..msg.chat_id_..':'..os.date('%d'))  
ChatType = 'sp' 
elseif id:match("^(%d+)") then
if not DevAbs:sismember(BANDE.."Abs:Users",msg.chat_id_) then
DevAbs:sadd(BANDE.."Abs:Users",msg.chat_id_)
end
ChatType = 'pv' 
else
ChatType = 'gp' 
...
if data.Ch_Member.Info_Dev-BANDE ~= true then
...
if not DevAbs:get(BANDE..'Abs:Start:Time'..msg.sender_user_id_) then
tdcli_function({ID="GetUser",user_id_=DevId},function(arg,dp)
tdcli_function({ID="GetUser",user_id_=BANDE},function(arg,dpbot)
local inline = {
{{text="✯ المطور .",url="t.me/"..(dp.username_ or "Dev-BANDE")}},
{{text="✯ السورس .",url="https://t.me/WaTaNTeaM"},{text="✯ لتنصيب بوت .",url="https://t.me/WaTaNTeaM/6109"}},{{text="✯ اضفني في مجموعتك .",url="t.me/"..dpbot.username_.."?startgroup=botstart"}}
}
local start = DevAbs:get(BANDE.."Abs:Start:Bot")
if start then 
Start_Source = start
else
...
DevAbs:setex(BANDE..'Abs:Start:Time'..msg.sender_user_id_,300,true)
return false
end 
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if not SecondSudo(msg) and not DevAbs:sismember(BANDE..'Abs:Ban:Pv',msg.sender_user_id_) and not DevAbs:get(BANDE..'Abs:Texting:Pv') then
tdcli_function({ID="GetUser",user_id_=DevId},function(arg,chat) 
Dev_Abs(msg.sender_user_id_, msg.id_, 1, '✯︙تم ارسال رسالتك الى [المطور](t.me/'..(chat.username_ or "Dev-BANDE")..')', 1, 'md') 
...
DevAbs:sadd(BANDE..'Abs:Ban:Pv',data.id_)  
return false  
end 
if text == 'الغاء الحظر' or text == 'الغاء حظر' then
local Text = '✯︙العضو ↫ ['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..data.id_..')'..'\n✯︙تم الغاء حظره من التواصل'
SendText(DevId,Text,msg.id_/2097152/0.5,'md') 
DevAbs:srem(BANDE..'Abs:Ban:Pv',data.id_)  
return false  
end 
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text and DevAbs:get(BANDE..'Abs:Start:Bots'..msg.sender_user_id_) then
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء حفظ كليشة الستارت', 1, 'md')
DevAbs:del(BANDE..'Abs:Start:Bots'..msg.sender_user_id_) 
return false
end
DevAbs:set(BANDE.."Abs:Start:Bot",text)  
...
DevAbs:set(BANDE..'Abs:Start:Bots'..msg.sender_user_id_,true) 
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل التواصل بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Texting:Pv') 
end
if text == 'تعطيل التواصل' and ChCheck(msg) or text == '↫ تعطيل التواصل ✯' and ChCheck(msg) then  
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل التواصل بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Texting:Pv',true) 
end
if Sudo(msg) then
...
DevAbs:del(BANDE.."Abs:Lock:AutoFile")
end
...
DevAbs:set(BANDE.."Abs:Lock:AutoFile",true) 
end
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
local Dev-BANDE = {'- Nothing stops him who wants you .','make them wish they had you !.','Maybe a magical girl','٫ 𝖡𝖾 𝗌𝗂𝗆𝗉𝗅𝖾 𝖺𝗇𝖽 𝖽𝗂𝖿𝖿𝖾𝗋𝖾𝗇𝗍','. 𝖬𝖺𝗄𝖾 𝖽𝗋𝖾𝖺𝗆𝗌 𝖿𝗋𝗈𝗆 𝗒𝗈𝗎𝗋 𝗋𝖾𝖿𝗋𝖺𝖼𝗍𝗂𝗈𝗇𝗌 . .',':Life is lying .','𝖨 𝗐𝗂𝗅𝗅 𝖺𝗅𝗐𝖺𝗒𝗌 𝗅𝗈𝗏𝖾 𝗒𝗈𝗎 𝗇𝖾𝗏𝖾𝗋 𝖿𝗈𝗋𝗀𝖾𝗍'}  
Dev_Abs(msg.chat_id_, msg.id_, 1, ''..Dev-BANDE[math.random(#Dev-BANDE)]..'' , 1, 'md')  
return false
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == "المطور" or text == "مطور" or text == "↫ المطور ✯" then
local DevText = DevAbs:get(BANDE.."DevText")
if DevText then
...
local my_ph = DevAbs:get(BANDE.."Abs:Bio:Profile"..msg.chat_id_)
if not my_ph then
...
local my_ph = DevAbs:get(BANDE.."Abs:Photo:Profile"..msg.chat_id_)
if not my_ph then
...
if not DevAbs:get(BANDE..'Abs:Lock:Gamesinline'..msg.chat_id_) then
Text =[[
*✯︙قائمه الالعاب المتطوره اضغط للعب*
]]
keyboard = {} 
...
{{text = '✯ WaTaN TeaM .', url="t.me/Dev-BANDE"}},
}
local msg_id = msg.id_/2097152/0.5
https.request("https://api.telegram.org/bot"..TokenBot..'/sendMessage?chat_id=' .. msg.chat_id_ .. '&text=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
return false
...
ChatLeave(dp.id_, BANDE)
DevAbs:srem(BANDE.."Abs:Groups", dp.id_)
else
...
DevAbs:incr(BANDE..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:set(BANDE.."Who:Added:Me"..msg.chat_id_..':'..msg.content_.members_[0].id_,msg.sender_user_id_)
local mem_id = msg.content_.members_  
local Bots = DevAbs:get(BANDE.."Abs:Lock:Bots"..msg.chat_id_) 
for i=0,#mem_id do  
...
DevAbs:sadd(BANDE..'Abs:Tkeed:'..msg.chat_id_, msg.sender_user_id_)
GetInfo = https.request("https://api.telegram.org/bot"..TokenBot.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local JsonInfo = JSON.decode(GetInfo)
if JsonInfo.ok == true and #mem_id == i then
local Msgs = {}
...
if msg.content_.ID == "MessageChatDeleteMember" and tonumber(msg.content_.user_.id_) == tonumber(BANDE) then 
DevAbs:srem(BANDE.."Abs:Groups", msg.chat_id_) 
...
if DevAbs:get(BANDE..'Abs:Lock:TagServr'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})    
end   
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then   
DevAbs:incr(BANDE..'Abs:EntryNumber'..msg.chat_id_..':'..os.date('%d'))  
elseif msg.content_.ID == "MessageChatDeleteMember" then   
DevAbs:incr(BANDE..'Abs:ExitNumber'..msg.chat_id_..':'..os.date('%d'))  
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text ==('تفعيل') and not SudoBot(msg) and ChCheck(msg) then
if not DevAbs:get(BANDE..'Abs:Lock:FreeBot'..BANDE) then
if ChatType == 'pv' then
...
for i=0 , #admins do
if Abs.members_[i].bot_info_ == false and Abs.members_[i].status_.ID == "ChatMemberStatusEditor" then
DevAbs:sadd(BANDE..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(BANDE..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
end
end,nil)   
else
DevAbs:sadd(BANDE..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
...
if Abs.members_[i].status_.ID == "ChatMemberStatusCreator" then
DevAbs:sadd(BANDE.."Abs:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevAbs:sadd(BANDE.."Abs:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(BANDE.."Abs:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevAbs:srem(BANDE.."Abs:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
end
end,nil)  
end 
end
end,nil)
if DevAbs:sismember(BANDE..'Abs:Groups',msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙المجموعه بالتاكيد مفعله', 1, 'md')
else
if tonumber(data.member_count_) < tonumber(DevAbs:get(BANDE..'Abs:Num:Add:Bot') or 0) and not SecondSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙عدد اعضاء المجموعه اقل من ↫ *'..(DevAbs:get(BANDE..'Abs:Num:Add:Bot') or 0)..'* عضو', 1, 'md')
return false
end
ReplyStatus(msg,result.id_,"ReplyBy","✯︙تم تفعيل المجموعه "..dp.title_)  
DevAbs:sadd(BANDE.."Abs:Groups",msg.chat_id_)
DevAbs:sadd(BANDE..'Abs:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_)
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
local Name1 = Name1:gsub("`","") 
...
DevAbs:set(BANDE.."Abs:Groups:Links"..msg.chat_id_,LinkGroup) 
...
DevAbs:set(BANDE..'Save:UserName'..msg.sender_user_id_,data.username_)
end;end,nil) 
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
local ReFalse = tostring(msg.chat_id_)
if not DevAbs:sismember(BANDE.."Abs:Groups",msg.chat_id_) and not ReFalse:match("^(%d+)") and not SudoBot(msg) then
...
if DevAbs:get(BANDE..'Abs:Lock:Robot'..msg.chat_id_) then
tdcli_function({ID="GetUser",user_id_=msg.sender_user_id_},function(arg,dp) 
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..dp.id_)
DevAbs:sadd(BANDE..'Abs:Tkeed:'..msg.chat_id_, dp.id_)
...
if DevAbs:get(BANDE.."Abs:Lock:Join"..msg.chat_id_) then
...
if not Bot(msg) then 
local filter = DevAbs:smembers(BANDE.."Abs:FilterPhoto"..msg.chat_id_)
...
if not Bot(msg) then 
local filter = DevAbs:smembers(BANDE.."Abs:FilterAnimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
if not Bot(msg) then 
local filter = DevAbs:smembers(BANDE.."Abs:FilterSteckr"..msg.chat_id_)
for k,v in pairs(filter) do
...
local DelGpRedRedods = DevAbs:get(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
local GetGpTexts = DevAbs:get(BANDE..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
if DelGpRedRedods == 'DelGpRedRedods' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙الرد ↫ '..msg.content_.text_..' للكلمه ↫ '..GetGpTexts..' تم حذفها',  1, "html")
DevAbs:del(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:srem(BANDE..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevAbs:get(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRedod' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙الكلمه ( '..msg.content_.text_..' ) تم حذفها',  1, "html")
DevAbs:del(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Text:GpTexts'..msg.content_.text_..msg.chat_id_)
DevAbs:srem(BANDE..'Abs:Manager:GpRedod'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevAbs:get(BANDE..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRed' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙الكلمه ( '..msg.content_.text_..' ) تم حذفها',  1, "html")
DevAbs:del(BANDE..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:srem(BANDE..'Abs:Manager:GpRed'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelAllRed = DevAbs:get(BANDE.."Abs:Add:AllRed"..msg.sender_user_id_)
if DelAllRed == 'DelAllRed' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙الكلمه ( '..msg.content_.text_..' ) تم حذفها',  1, "html")
DevAbs:del(BANDE.."Abs:Add:AllRed"..msg.sender_user_id_)
DevAbs:del(BANDE.."Abs:Gif:AllRed"..msg.content_.text_)
DevAbs:del(BANDE.."Abs:Voice:AllRed"..msg.content_.text_)
DevAbs:del(BANDE.."Abs:Audio:AllRed"..msg.content_.text_)
DevAbs:del(BANDE.."Abs:Photo:AllRed"..msg.content_.text_)
DevAbs:del(BANDE.."Abs:Stecker:AllRed"..msg.content_.text_)
DevAbs:del(BANDE.."Abs:Video:AllRed"..msg.content_.text_)
DevAbs:del(BANDE.."Abs:File:AllRed"..msg.content_.text_)
DevAbs:del(BANDE.."Abs:Text:AllRed"..msg.content_.text_)
DevAbs:del(BANDE.."Abs:Sudo:AllRed",msg.content_.text_)
return false
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text and text:match("^(.*)$") then
local SaveGpRedod = DevAbs:get(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRedod == 'SaveGpRedod' then
local GetGpTexts = DevAbs:get(BANDE..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
local List = DevAbs:smembers(BANDE..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_)
if text == "الغاء" then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙✯︙تم الغاء عملية حفظ الردود المتعدده للامر ↫ "..GetGpTexts ,  1, "md")
DevAbs:del(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
DevAbs:srem(BANDE..'Abs:Manager:GpRedod'..msg.chat_id_,GetGpTexts)
return false
end
Text = text:gsub('"',""):gsub('"',""):gsub("`",""):gsub("*","")
DevAbs:sadd(BANDE..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_,Text)
if #List == 4 then 
...
if text and not DevAbs:get(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_) then
if DevAbs:sismember(BANDE..'Abs:Manager:GpRedod'..msg.chat_id_,text) then
local Dev-BANDE =  DevAbs:smembers(BANDE..'Abs:Text:GpTexts'..text..msg.chat_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '['..Dev-BANDE[math.random(#Dev-BANDE)]..']' , 1, 'md')  
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text and text:match("^(.*)$") then
local SaveKt = DevAbs:get(BANDE..'Abs:Add:Kt'..msg.sender_user_id_..msg.chat_id_)
if SaveKt == 'SaveKt' then
local GetKtTexts = DevAbs:get(BANDE..'Abs:Add:KtTexts'..msg.sender_user_id_..msg.chat_id_)
local List = DevAbs:smembers(BANDE..'Abs:Text:KtTexts'..GetKtTexts..msg.chat_id_)
if text == "الغاء" then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙✯︙تم الغاء عملية حفظ اسئلة الكت  ↫ "..GetKtTexts ,  1, "md")
DevAbs:del(BANDE..'Abs:Add:Kt'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Text:KtTexts'..GetKtTexts..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Add:KtTexts'..msg.sender_user_id_..msg.chat_id_)
DevAbs:srem(BANDE..'Abs:Sudo:Kt'..msg.chat_id_,GetKtTexts)
return false
end
Text = text:gsub('"',""):gsub('"',""):gsub("`",""):gsub("*","")
DevAbs:sadd(BANDE..'Abs:Text:KtTexts'..GetKtTexts..msg.chat_id_,Text)
if #List == 100 then 
...
if text and not DevAbs:get(BANDE..'Abs:Add:Kt'..msg.sender_user_id_..msg.chat_id_) then
if DevAbs:sismember(BANDE..'Abs:Sudo:Kt'..msg.chat_id_,text) then
local Dev-BANDE =  DevAbs:smembers(BANDE..'Abs:Text:KtTexts'..text..msg.chat_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '['..Dev-BANDE[math.random(#Dev-BANDE)]..']' , 1, 'md')  
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then 
local SaveGpRed = DevAbs:get(BANDE..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRed == 'SaveGpRed' then 
if text == 'الغاء' then
local DelManagerRep = DevAbs:get(BANDE..'DelManagerRep'..msg.chat_id_)
DevAbs:srem(BANDE..'Abs:Manager:GpRed'..msg.chat_id_,DelManagerRep)
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء حفظ الرد', 1, 'md')
DevAbs:del(BANDE..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(BANDE..'DelManagerRep'..msg.chat_id_)
return false
end
DevAbs:del(BANDE..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
local SaveGpRed = DevAbs:get(BANDE..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
if msg.content_.video_ then DevAbs:set(BANDE..'Abs:Video:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then DevAbs:set(BANDE..'Abs:File:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then DevAbs:set(BANDE..'Abs:Stecker:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_) 
end 
if msg.content_.voice_ then DevAbs:set(BANDE..'Abs:Voice:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_) 
end
if msg.content_.audio_ then DevAbs:set(BANDE..'Abs:Audio:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_) 
end
...
DevAbs:set(BANDE..'Abs:Photo:GpRed'..SaveGpRed..msg.chat_id_, photo_in_group) 
end
if msg.content_.animation_ then DevAbs:set(BANDE..'Abs:Gif:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_) 
end 
if msg.content_.text_ then
DevAbs:set(BANDE..'Abs:Text:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.text_)
end 
...
if msg.content_.text_ and not DevAbs:get(BANDE..'Abs:Lock:GpRed'..msg.chat_id_) then 
if DevAbs:get(BANDE..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(BANDE..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(BANDE..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(BANDE..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(BANDE..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(BANDE..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(BANDE..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(BANDE..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(BANDE..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(BANDE..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(BANDE..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(BANDE..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(BANDE..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(BANDE..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_))
end
if DevAbs:get(BANDE..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_) then
function Dev-BANDE(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevAbs:get(BANDE..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevAbs:get(BANDE..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_)
...
getUser(msg.sender_user_id_, Dev-BANDE)
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
text = msg.content_.text_
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then
local SaveAllRed = DevAbs:get(BANDE.."Abs:Add:AllRed"..msg.sender_user_id_)
if SaveAllRed == 'SaveAllRed' then
if text == 'الغاء' then
local DelSudoRep = DevAbs:get(BANDE..'DelSudoRep')
DevAbs:del(BANDE.."Abs:Sudo:AllRed",DelSudoRep)
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء حفظ الرد', 1, 'md')
DevAbs:del(BANDE.."Abs:Add:AllText"..msg.sender_user_id_)
DevAbs:del(BANDE.."Abs:Add:AllRed"..msg.sender_user_id_)
DevAbs:del(BANDE.."DelSudoRep")
return false
end
DevAbs:del(BANDE.."Abs:Add:AllRed"..msg.sender_user_id_)
local SaveAllRed = DevAbs:get(BANDE.."Abs:Add:AllText"..msg.sender_user_id_)
if msg.content_.video_ then
DevAbs:set(BANDE.."Abs:Video:AllRed"..SaveAllRed, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then
DevAbs:set(BANDE.."Abs:File:AllRed"..SaveAllRed, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then
DevAbs:set(BANDE.."Abs:Stecker:AllRed"..SaveAllRed, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
DevAbs:set(BANDE.."Abs:Voice:AllRed"..SaveAllRed, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.audio_ then
DevAbs:set(BANDE.."Abs:Audio:AllRed"..SaveAllRed, msg.content_.audio_.audio_.persistent_id_)
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
...
DevAbs:set(BANDE.."Abs:Photo:AllRed"..SaveAllRed, photo_in_all_groups)
end
if msg.content_.animation_ then
DevAbs:set(BANDE.."Abs:Gif:AllRed"..SaveAllRed, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
DevAbs:set(BANDE.."Abs:Text:AllRed"..SaveAllRed, msg.content_.text_)
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم حفظ الرد الجديد', 1, 'md') 
DevAbs:del(BANDE.."Abs:Add:AllText"..msg.sender_user_id_)
DevAbs:del(BANDE..'DelSudoRep')
return false end end
if msg.content_.text_ and not DevAbs:get(BANDE..'Abs:Lock:AllRed'..msg.chat_id_) then
if DevAbs:get(BANDE.."Abs:Video:AllRed"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(BANDE.."Abs:Video:AllRed"..msg.content_.text_))
end
if DevAbs:get(BANDE.."Abs:File:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(BANDE.."Abs:File:AllRed"..msg.content_.text_))
end
if DevAbs:get(BANDE.."Abs:Voice:AllRed"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(BANDE.."Abs:Voice:AllRed"..msg.content_.text_))
end
if DevAbs:get(BANDE.."Abs:Audio:AllRed"..msg.content_.text_)  then
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(BANDE.."Abs:Audio:AllRed"..msg.content_.text_))
end
if DevAbs:get(BANDE.."Abs:Photo:AllRed"..msg.content_.text_)  then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(BANDE.."Abs:Photo:AllRed"..msg.content_.text_))
end
if  DevAbs:get(BANDE.."Abs:Gif:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(BANDE.."Abs:Gif:AllRed"..msg.content_.text_))
end
if DevAbs:get(BANDE.."Abs:Stecker:AllRed"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(BANDE.."Abs:Stecker:AllRed"..msg.content_.text_))
end
if DevAbs:get(BANDE.."Abs:Text:AllRed"..msg.content_.text_) then
function Dev-BANDE(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevAbs:get(BANDE..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevAbs:get(BANDE.."Abs:Text:AllRed"..msg.content_.text_)
local Text = Text:gsub('#username',(username or 'لا يوجد')) 
local Text = Text:gsub('#name','['..result.first_name_..']')
local Text = Text:gsub('#id',msg.sender_user_id_)
local Text = Text:gsub('#edit',edit_msg)
...
if Type == "keed" and not DevAbs:sismember(BANDE..'Abs:Tkeed:'..msg.chat_id_, msg.sender_user_id_) then
...
if Type == "mute" and not DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_, msg.sender_user_id_) then
DevAbs:sadd(BANDE..'Abs:Muted:'..msg.chat_id_,msg.sender_user_id_)
my_ide = msg.sender_user_id_
msgm = msg.id_
local num = 100
for i=1,tonumber(num) do
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
--       Spam Check       --
if not VipMem(msg) and msg.content_.ID ~= "MessageChatAddMembers" and DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") then 
if msg.sender_user_id_ ~= BANDE then
floods = DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") or "nil"
Num_Msg_Max = DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
Time_Spam = DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 5
local post_count = tonumber(DevAbs:get(BANDE.."Abs:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_) or 0)
if post_count > tonumber(DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5) then 
local ch = msg.chat_id_
local type = DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") 
NotSpam(msg,type)  
end
DevAbs:setex(BANDE.."Abs:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_, tonumber(DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 3), post_count+1) 
local edit_id = data.text_ or "nil"  
Num_Msg_Max = 5
if DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") then
Num_Msg_Max = DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") 
end
if DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") then
Time_Spam = DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") 
end 
end
end 
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
if Constructor(msg) or tonumber(msg.sender_user_id_) == tonumber(BANDE) then
DevAbs:set(BANDE..'Abs:PinnedMsg'..msg.chat_id_,msg.content_.message_id_)
else
local pin_id = DevAbs:get(BANDE..'Abs:PinnedMsg'..msg.chat_id_)
if pin_id and DevAbs:get(BANDE..'Abs:Lock:Pin'..msg.chat_id_) then
pinmsg(msg.chat_id_,pin_id,0)
end
end
end
if DevAbs:get(BANDE..'Abs:viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
DevAbs:del(BANDE..'Abs:viewget'..msg.sender_user_id_)
else
...
if DevAbs:get(BANDE..'Abs:Lock:Forwards'..msg.chat_id_) then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
if DevAbs:get(BANDE..'Abs:Lock:Photo'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
if DevAbs:get(BANDE..'Abs:Lock:Links'..msg.chat_id_) then
if msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if DevAbs:get(BANDE..'Abs:Lock:Tags'..msg.chat_id_) then
if msg.content_.caption_:match("@") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("#") then
if DevAbs:get(BANDE..'Abs:Lock:Hashtak'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://") or msg.content_.caption_:match(".[Ii][Rr]") or msg.content_.caption_:match(".[Cc][Oo][Mm]") or msg.content_.caption_:match(".[Oo][Rr][Gg]") or msg.content_.caption_:match(".[Ii][Nn][Ff][Oo]") or msg.content_.caption_:match("[Ww][Ww][Ww].") or msg.content_.caption_:match(".[Xx][Yy][Zz]") or msg.content_.caption_:match(".[Tt][Kk]") or msg.content_.ID == "MessageEntityTextUrl" or msg.content_.ID == "MessageEntityUrl" then
if DevAbs:get(BANDE..'Abs:Lock:WebLinks'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[\216-\219][\128-\191]") then
if DevAbs:get(BANDE..'Abs:Lock:Arabic'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]") then
if DevAbs:get(BANDE..'Abs:Lock:English'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if DevAbs:get(BANDE..'Abs:Lock:Markdown'..msg.chat_id_) then
if not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevAbs:get(BANDE..'Abs:Lock:Document'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if not VipMem(msg) then
if DevAbs:get(BANDE..'Abs:Lock:Inline'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
elseif msg.content_.ID == "MessageSticker" then
if not VipMem(msg) then
if DevAbs:get(BANDE..'Abs:Lock:Stickers'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevAbs:get(BANDE..'Abs:Groups:Welcomes'..msg.chat_id_) then
Welcomes = DevAbs:get(BANDE..'Abs:Groups:Welcomes'..msg.chat_id_)
else
Welcomes = '• نورت حبي \n• firstname \n• @username'
end
local Welcomes = Welcomes:gsub('"',"") Welcomes = Welcomes:gsub("'","") Welcomes = Welcomes:gsub(",","") Welcomes = Welcomes:gsub("*","") Welcomes = Welcomes:gsub(";","") Welcomes = Welcomes:gsub("`","") Welcomes = Welcomes:gsub("{","") Welcomes = Welcomes:gsub("}","") 
local Welcomes = Welcomes:gsub('firstname',('['..result.first_name_..']' or ''))
local Welcomes = Welcomes:gsub('username',('[@'..result.username_..']' or '[@WaTaNTeaM]'))
Dev_Abs(msg.chat_id_, msg.id_, 1, Welcomes, 1, 'md')
end 
if DevAbs:get(BANDE.."Abs:Lock:Welcome"..msg.chat_id_) then
getUser(msg.sender_user_id_,get_welcome)
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
--      New User Add      --
elseif msg.content_.ID == "MessageChatAddMembers" then
if not DevAbs:get(BANDE..'Abs:Lock:BotWelcome') then 
tdcli_function ({ID = "GetUserProfilePhotos",user_id_ = BANDE,offset_ = 0,limit_ = 1},function(extra,Abs,success) 
for i=0,#msg.content_.members_ do    
BotWelcome = msg.content_.members_[i].id_    
if BotWelcome and BotWelcome == tonumber(BANDE) then 
if DevAbs:sismember(BANDE..'Abs:Groups',msg.chat_id_) then BotText = "مفعله في السابق\n✯︙ارسل ↫ الاوامر واستمتع بالمميزيات" else BotText = "معطله يجب رفعي مشرف\n✯︙بعد ذلك يرجى ارسال امر ↫ تفعيل\n✯︙سيتم رفع الادمنيه والمنشئ تلقائيا" end 
if DevAbs:get(BANDE.."Abs:Text:BotWelcome") then AbsText = DevAbs:get(BANDE.."Abs:Text:BotWelcome") else AbsText = "✯︙مرحبا انا بوت اسمي "..NameBot.."\n✯︙حالة المجموعه ↫ "..BotText.."\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉" end 
if DevAbs:get(BANDE.."Abs:Photo:BotWelcome") then AbsPhoto = DevAbs:get(BANDE.."Abs:Photo:BotWelcome") elseif Abs.photos_[0] then AbsPhoto = Abs.photos_[0].sizes_[1].photo_.persistent_id_ else AbsPhoto = nil end 
...
if DevAbs:get(BANDE..'Abs:Lock:Contact'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
if DevAbs:get(BANDE..'Abs:Lock:Music'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(BANDE..'Abs:Lock:Voice'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(BANDE..'Abs:Lock:Location'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
return
end
if msg.content_.caption_ then
...
if DevAbs:get(BANDE..'Abs:Lock:Videos'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(BANDE..'Abs:Lock:Gifs'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(BANDE..'Abs:Lock:Text'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
...
if not DevAbs:get(BANDE..'Abs:Spam:Text'..msg.chat_id_) then
sens = 400
else
sens = tonumber(DevAbs:get(BANDE..'Abs:Spam:Text'..msg.chat_id_))
end
if DevAbs:get(BANDE..'Abs:Lock:Spam'..msg.chat_id_) and string.len(msg.content_.text_) > (sens) or ctrl_chars > (sens) or real_digits > (sens) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) then
if text == "الغاء" then
send(msg.chat_id_,msg.id_,"✯︙تم الغاء حفظ الرابط")       
DevAbs:del(BANDE.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) 
return false
end
if msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)") then
local Link = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
DevAbs:set(BANDE.."Abs:Groups:Links"..msg.chat_id_,Link)
...
if DevAbs:get('Dev-BANDE:'..BANDE.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevAbs:del('Dev-BANDE:'..BANDE..'id:user'..msg.chat_id_)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء الامر', 1, 'md')
DevAbs:del('Dev-BANDE:'..BANDE.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del('Dev-BANDE:'..BANDE.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevAbs:get('Dev-BANDE:'..BANDE..'id:user'..msg.chat_id_)  
DevAbs:incrby(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..iduserr,numadded)
Dev_Abs(msg.chat_id_, msg.id_,  1, "✯︙تم اضافة "..numadded..' رساله', 1, 'md')
DevAbs:del('Dev-BANDE:'..BANDE..'id:user'..msg.chat_id_) 
end
end
if text and Constructor(msg) then 
if DevAbs:get('Dev-BANDE:'..BANDE.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevAbs:del('Dev-BANDE:'..BANDE..'ids:user'..msg.chat_id_)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء الامر', 1, 'md')
DevAbs:del('Dev-BANDE:'..BANDE.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del('Dev-BANDE:'..BANDE.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevAbs:get('Dev-BANDE:'..BANDE..'ids:user'..msg.chat_id_)  
DevAbs:incrby(BANDE..'Abs:GamesNumber'..msg.chat_id_..iduserr,numadded)  
...
if not DevAbs:get(BANDE.."Abs:Lock:Fshar"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if DevAbs:get(BANDE.."Abs:Lock:Farsi"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ReplyStatus(msg,msg.sender_user_id_,"WrongWay","✯︙ممنوع التكلم بالغه الفارسيه هنا")  
end end
if text and (text:match("ڬ") or text:match("ٺ") or text:match("چ") or text:match("ڇ") or text:match("ڿ") or text:match("ڀ") or text:match("ڎ") or text:match("ݫ") or text:match("ژ") or text:match("ڟ") or text:match("ݜ") or text:match("ڸ") or text:match("پ") or text:match("۴") or text:match("مک") or text:match("زدن") or text:match("دخترا") or text:match("دیوث") or text:match("کلیپشن") or text:match("خوششون") or text:match("میدا") or text:match("که") or text:match("بدانیم") or text:match("باید") or text:match("زناشویی") or text:match("آموزش") or text:match("راحتی") or text:match("خسته") or text:match("بیام") or text:match("بپوشم") or text:match("كرمه")) then
if DevAbs:get(BANDE.."Abs:Lock:FarsiBan"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ChatKick(msg.chat_id_, msg.sender_user_id_)
end end 
if text and (text:match("خره بالله") or text:match("خبربك") or text:match("كسدينربك") or text:match("خرب بالله") or text:match("خرب الله") or text:match("خره بربك") or text:match("الله الكواد") or text:match("خره بمحمد") or text:match("كسم الله") or text:match("كسم ربك") or text:match("كسربك") or text:match("كسختالله") or text:match("كسخت الله") or text:match("خره بدينك") or text:match("خرهبدينك") or text:match("كسالله") or text:match("خربالله")) then
if not DevAbs:get(BANDE.."Abs:Lock:Kfr"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if not DevAbs:get(BANDE.."Abs:Lock:Taf"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './BANDE.lua', '✯︙نسخة ملف سورس وطن',dl_cb, nil)
end end
if text == 'جلب النسخه' and SudoBot(msg) then
local Groups = DevAbs:smembers(BANDE..'Abs:Groups')  
local UsersBot = DevAbs:smembers(BANDE..'Abs:Users')  
local Get_Json = '{"BotId": '..BANDE..','  
if #UsersBot ~= 0 then 
...
local Malk = DevAbs:smembers(BANDE..":Abs:AbsConstructor:"..v)
local President = DevAbs:smembers(BANDE.."Abs:BasicConstructor:"..v)
local Constructor = DevAbs:smembers(BANDE..":Abs:Constructor:"..v)
local Manager = DevAbs:smembers(BANDE.."Abs:Managers:"..v)
local Admin = DevAbs:smembers(BANDE.."Abs:Admins:"..v)
local Vips = DevAbs:smembers(BANDE.."Abs:VipMem:"..v)
if k == 1 then
Get_Json = Get_Json..'"'..v..'":{'
else
Get_Json = Get_Json..',"'..v..'":{'
...
Get_Json = Get_Json..'}}'
local File = io.open('./'..BANDE..'.json', "w")
File:write(Get_Json)
File:close()
return sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './'..BANDE..'.json', '',dl_cb, nil)
end
...
local List = DevAbs:smembers(BANDE..'Abs:Groups') 
local BotName = (DevAbs:get(BANDE.."Abs:NameBot") or 'وطن')
local GetJson = '{"BotId": '..BANDE..',"BotName": "'..BotName..'","GroupsList":{'  
for k,v in pairs(List) do 
LinkGroups = DevAbs:get(BANDE.."Abs:Groups:Links"..v)
Welcomes = DevAbs:get(BANDE..'Abs:Groups:Welcomes'..v) or ''
Welcomes = Welcomes:gsub('"',"") Welcomes = Welcomes:gsub("'","") Welcomes = Welcomes:gsub(",","") Welcomes = Welcomes:gsub("*","") Welcomes = Welcomes:gsub(";","") Welcomes = Welcomes:gsub("`","") Welcomes = Welcomes:gsub("{","") Welcomes = Welcomes:gsub("}","") 
AbsConstructors = DevAbs:smembers(BANDE..'Abs:AbsConstructor:'..v)
Constructors = DevAbs:smembers(BANDE..'Abs:BasicConstructor:'..v)
BasicConstructors = DevAbs:smembers(BANDE..'Abs:Constructor:'..v)
Managers = DevAbs:smembers(BANDE..'Abs:Managers:'..v)
Admis = DevAbs:smembers(BANDE..'Abs:Admins:'..v)
Vips = DevAbs:smembers(BANDE..'Abs:VipMem:'..v)
if k == 1 then
GetJson = GetJson..'"'..v..'":{'
else
GetJson = GetJson..',"'..v..'":{'
...
File:write(GetJson)
File:close()
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './'..BANDE..'.json', '✯︙يحتوي الملف على ↫ '..#List..' مجموعه',dl_cb, nil)
io.popen('rm -rf ./'..BANDE..'.json')
end
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE.."SET:GAME"..msg.chat_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 6 then
Dev_Abs( msg.chat_id_, msg.id_, 1,"✯︙يوجد فقط ( 6 ) اختيارات\n✯︙ارسل اختيارك مره اخرى", 1, "md")    
return false  end 
local GETNUM = DevAbs:get(BANDE.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
DevAbs:del(BANDE.."SET:GAME"..msg.chat_id_)   
Dev_Abs( msg.chat_id_, msg.id_, 1,'✯︙*المحيبس باليد رقم* ↫ '..NUM..'\n✯︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*', 1, "md") 
DevAbs:incrby(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_,5)  
...
if DevAbs:get(BANDE..'DevAbs4'..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_, "✯︙تم الغاء الامر")
DevAbs:del(BANDE..'DevAbs4'..msg.sender_user_id_)
return false  end 
DevAbs:del(BANDE..'DevAbs4'..msg.sender_user_id_)
...
DevAbs:set(BANDE..'Abs:ChId',data.id_)
else
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
DevAbs:del(BANDE.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء الامر', 1, 'md')
return false 
end 
DevAbs:del(BANDE.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
local DevText = msg.content_.text_:match("(.*)")
DevAbs:set(BANDE.."DevText", DevText)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ كليشة المطور", 1, "md")
end
if DevAbs:get(BANDE..'Abs:NameBot'..msg.sender_user_id_) == 'msg' then
if text and text:match("^الغاء$") then 
DevAbs:del(BANDE..'Abs:NameBot'..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء الامر', 1, 'md')
return false 
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم حفظ اسم البوت ', 1, 'html')
DevAbs:del(BANDE..'Abs:NameBot'..msg.sender_user_id_)
DevAbs:set(BANDE..'Abs:NameBot', text)
return false 
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if msg.chat_id_ then
...
NameBot = (DevAbs:get(BANDE..'Abs:NameBot') or 'وطن')
local Dev-BANDE = {' كول حبيبي ؟ اني '..NameBot..' ',' وياك القميل '..NameBot..' ',' اسمي القميل '..NameBot..' '}
DevAbs2 = math.random(#Dev-BANDE) 
Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE[DevAbs2] , 1, 'html') 
return false
end
if text == 'اسم البوت' or text == 'البوت شنو اسمه' or text == 'شسمه البوت' or text == 'البوت شسمه' then
NameBot = (DevAbs:get(BANDE..'Abs:NameBot') or 'وطن') 
local Dev-BANDE = {"اسمي القميل "..NameBot.." "} 
DevAbs2 = math.random(#Dev-BANDE) 
Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE[DevAbs2] , 1, 'html') 
return false
end
if text and text == (DevAbs:get(BANDE..'Abs:NameBot') or 'وطن') then 
NameBot = (DevAbs:get(BANDE..'Abs:NameBot') or 'وطن')
...
if tonumber((DevAbs:get(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙لم تربح اي نقطه\n✯︙ارسل ↫ الالعاب للعب', 1, 'md')
else 
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙عدد النقاط التي ربحتها ↫ '..(DevAbs:get(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_)), 1, 'md')
end
end
if text ==  'حذف نقاطي' and ChCheck(msg) or text ==  'مسح نقاطي' and ChCheck(msg) then DevAbs:del(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم حذف جميع نقاطك', 1, 'md') end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == 'سمايلات' and ChCheck(msg) or text == 'السمايلات' and ChCheck(msg) or text == '↫ سمايلات ✯' and ChCheck(msg) then
if not DevAbs:get(BANDE..'Abs:Lock:Games'..msg.chat_id_) then
DevAbs2 = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','🍠','🥐','🍞','🥖','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍟','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🏆','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🛎','📤','🎗','🏵','🎖','🏆','🥌','??','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔️','🛡','🔮','🌡','💣','⏱','🛢','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(BANDE..'Abs:GameNum'..msg.chat_id_,name)
DevAbs:del(BANDE..'Abs:Games:Ids'..msg.chat_id_)
...
name = string.gsub(name,'🎚','🎚')
name = string.gsub(name,'☎️','☎️')
Dev-BANDE = '✯︙اول واحد يدز هذا السمايل يربح ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
return false
end end
if text == DevAbs:get(BANDE..'Abs:GameNum'..msg.chat_id_) and not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then 
Dev-BANDE = '✯︙مبروك لقد ربحت في اللعبه \n✯︙ارسل ↫ سمايلات للعب مره اخرى'
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
DevAbs:incrby(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
end
DevAbs:set(BANDE..'Abs:Games:Ids'..msg.chat_id_,true)
end
...
Dev-BANDE = '✯︙اول واحد يرتبها يربح ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
return false
end end
if text == DevAbs:get(BANDE..'Abs:GameNum'..msg.chat_id_) and not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then
...
if not DevAbs:get(BANDE..'Abs:Lock:Games'..msg.chat_id_) then
Num = math.random(1,6)
DevAbs:set(BANDE.."GAMES"..msg.chat_id_,Num) 
TEST = [[
➀     ➁     ➂     ➃     ➄     ➅
↓     ↓     ↓     ↓     ↓     ↓
👊 ‹› 👊🏻 ‹› 👊🏼 ‹› 👊🏽 ‹› 👊🏾 ‹› 👊🏿
...
DevAbs:setex(BANDE.."SET:GAME"..msg.chat_id_, 100, true)  
return false  
end end
...
Dev-BANDE = '✯︙اول واحد يحلها يربح ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
return false
end end
if text == DevAbs:get(BANDE..'Abs:GameNum'..msg.chat_id_) and not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(BANDE..'Abs:GameNum2'..msg.chat_id_,name)
DevAbs:del(BANDE..'Abs:Games:Ids'..msg.chat_id_)
...
Dev-BANDE = '✯︙ما معنى هذا السمايل :؟ ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
return false
end end
if text == DevAbs:get(BANDE..'Abs:GameNum2'..msg.chat_id_) and not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then 
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(BANDE..'Abs:GameNum3'..msg.chat_id_,name)
DevAbs:del(BANDE..'Abs:Games:Ids'..msg.chat_id_)
...
Dev-BANDE = '✯︙ما هو عكس كلمة ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
return false
end end
if text == DevAbs:get(BANDE..'Abs:GameNum3'..msg.chat_id_) and not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then 
...
DevAbs2 = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','📥','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧚‍♂️','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕓','🕤','⌛️','📅','👩‍⚖️','👨‍🎨'};
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(BANDE..'Abs:GameNum4'..msg.chat_id_,name)
DevAbs:del(BANDE..'Abs:Games:Ids'..msg.chat_id_)
name = string.gsub(name,'😸','😹😹😹😸😹😹😹😹')
name = string.gsub(name,'☠️','💀💀💀☠️💀💀💀💀')
name = string.gsub(name,'🐼','👻👻👻👻👻👻👻🐼')
...
name = string.gsub(name,'👩‍⚖️','👨‍⚖️👨‍⚖️👨‍⚖️👨‍⚖️👨‍⚖️👩‍⚖️👨‍⚖️👨‍⚖️')
name = string.gsub(name,'👨‍🎨','👩‍🎨👩‍🎨👨‍🎨👩‍🎨👩‍🎨👩‍🎨👩‍🎨👩‍🎨')
Dev-BANDE = '✯︙اول واحد يطلع المختلف يربح\n{'..name..'} '
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
return false
end end
if text == DevAbs:get(BANDE..'Abs:GameNum4'..msg.chat_id_) and not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then
if not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then 
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(BANDE..'Abs:GameNum5'..msg.chat_id_,name)
DevAbs:del(BANDE..'Abs:Games:Ids'..msg.chat_id_)
...
Dev-BANDE = '✯︙اكمل المثال التالي ↫ ['..name..']'
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
return false
end end
if text == DevAbs:get(BANDE..'Abs:GameNum5'..msg.chat_id_) then
if not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(BANDE..'Abs:GameNum5'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(BANDE..'Abs:GameNum6'..msg.chat_id_,name)
DevAbs:del(BANDE..'Abs:Games:Ids'..msg.chat_id_)
...
Dev-BANDE = '✯︙اكمل المعادله التاليه ↫ ⤈\n{'..name..'} '
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
return false
end end
if text == DevAbs:get(BANDE..'Abs:GameNum6'..msg.chat_id_) then
if not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(BANDE..'Abs:GameNum6'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(BANDE..'Abs:GameNum7'..msg.chat_id_,name)
DevAbs:del(BANDE..'Abs:Games:Ids'..msg.chat_id_)
...
Dev-BANDE = '✯︙ما معنى كلمة ↫ '..name
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md')
return false
end end
if text == DevAbs:get(BANDE..'Abs:GameNum7'..msg.chat_id_) then
if not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(BANDE..'Abs:GameNum7'..msg.chat_id_)
...
name = DevAbs2[math.random(#DevAbs2)]
DevAbs:set(BANDE..'Abs:GameNum8'..msg.chat_id_,name)
DevAbs:del(BANDE..'Abs:Games:Ids'..msg.chat_id_)
...
if text == DevAbs:get(BANDE..'Abs:GameNum8'..msg.chat_id_) then
if not DevAbs:get(BANDE..'Abs:Games:Ids'..msg.chat_id_) then 
DevAbs:incrby(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_, 1)  
DevAbs:del(BANDE..'Abs:GameNum8'..msg.chat_id_)
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 20 then
Dev_Abs(msg.chat_id_, msg.id_, 1,"✯︙عذرا لا يمكنك تخمين عدد اكبر من الـ20 خمن رقم ما بين الـ1 والـ20", 1, 'md')
return false  end 
local GETNUM = DevAbs:get(BANDE.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
DevAbs:del(BANDE..'Set:Num'..msg.chat_id_..msg.sender_user_id_)
DevAbs:del(BANDE.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_)   
DevAbs:incrby(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_,5)  
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙*التخمين الصحيح هو* ↫ '..NUM..'\n✯︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*', 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
DevAbs:incrby(BANDE..'Set:Num'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(DevAbs:get(BANDE..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
DevAbs:del(BANDE..'Set:Num'..msg.chat_id_..msg.sender_user_id_)
DevAbs:del(BANDE.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_)   
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙*التخمين الصحيح هو* ↫ '..GETNUM..'\n✯︙*للاسف لقد خسرت حاول مره اخرى لتخمين الرقم الصحيح*', 1, 'md')
else
if tonumber(DevAbs:get(BANDE..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) == 1 then
SetNum = 'محاولتان فقط'
elseif tonumber(DevAbs:get(BANDE..'Set:Num'..msg.chat_id_..msg.sender_user_id_)) == 2 then
...
DevAbs:set(BANDE.."GAMES:NUM"..msg.chat_id_,Num) 
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙اهلا بك عزيزي في لعبة التخمين ↫ ⤈\n ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n✯︙سيتم تخمين عدد ما بين الـ1 والـ20 اذا تعتقد انك تستطيع الفوز جرب واللعب الان .\n✯︙ملاحظه لديك ثلاث محاولات فقط فكر قبل ارسال تخمينك !', 1, 'md')
DevAbs:setex(BANDE.."GAME:TKMEN"..msg.chat_id_.."" .. msg.sender_user_id_, 100, true)  
return false  
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
if not DevAbs:get(BANDE..'Abs:Lock:Games'..msg.chat_id_) then
DevAbs:del(BANDE.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_) 
DevAbs:del(BANDE..'Abs:ListRolet'..msg.chat_id_)  
DevAbs:setex(BANDE.."Abs:StartRolet"..msg.chat_id_..msg.sender_user_id_,3600,true)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙حسنا لنلعب , ارسل عدد اللاعبين للروليت .', 1, 'md')
return false  
end
end
if text and text:match("^(%d+)$") and DevAbs:get(BANDE.."Abs:StartRolet"..msg.chat_id_..msg.sender_user_id_) then
if text == "1" then
Text = "✯︙لا استطيع بدء اللعبه بلاعب واحد فقط"
else
DevAbs:set(BANDE.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_,text)  
Text = '✯︙تم بدء تسجيل اللسته يرجى ارسال المعرفات \n✯︙الفائز يحصل على 5 نقاط عدد المطلوبين ↫ '..text..' لاعب'
end
DevAbs:del(BANDE.."Abs:StartRolet"..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_,msg.id_,Text)
return false
end
if text and text:match('^(@[%a%d_]+)$') and DevAbs:get(BANDE.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_) then 
if DevAbs:sismember(BANDE..'Abs:ListRolet'..msg.chat_id_,text) then
...
DevAbs:sadd(BANDE..'Abs:ListRolet'..msg.chat_id_,text)
local CountAdd = DevAbs:get(BANDE.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_)
local CountAll = DevAbs:scard(BANDE..'Abs:ListRolet'..msg.chat_id_)
local CountUser = CountAdd - CountAll
if tonumber(CountAll) == tonumber(CountAdd) then 
DevAbs:del(BANDE.."Abs:NumRolet"..msg.chat_id_..msg.sender_user_id_) 
DevAbs:setex(BANDE.."Abs:WittingStartRolet"..msg.chat_id_..msg.sender_user_id_,1400,true) 
...
local Tsl = DevAbs:get(BANDE..'Abs:Tsl')
local Text = [[
...
elseif Constructor(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..Abs},{text="تنزيل ادمن",callback_data=msg.sender_user_id_..":RemAdmin:"..Abs}},{{text="تنزيل منظف",callback_data=msg.sender_user_id_..":RemCleaner:"..Abs},{text="تنزيل مدير",callback_data=msg.sender_user_id_..":RemManager:"..Abs}},{{text = '⌁ BANDE TeAM.',url="t.me/WaTaNTeaM"}}}
elseif Manager(data) then
keyboard.inline_keyboard = {{{text="تنزيل مميز",callback_data=msg.sender_user_id_..":RemMem:"..Abs},{text="تنزيل ادمن",callback_data=msg.sender_user_id_..":RemAdmin:"..Abs}}}
elseif Admin(data) then
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == "الرابط" then
local Link = DevAbs:get(BANDE..'Abs:Link')
local Text = [[
...
if not DevAbs:get(BANDE..'Abs:Lock:Games'..msg.chat_id_) then
local Games = DevAbs:get(BANDE..'Abs:Games')
local Text = [[
...
DevAbs0 = (DevAbs:get(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) * 50)
DevAbs:incrby(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_,DevAbs0)
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙تم بيع '..(DevAbs:get(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_))..' من نقاطك\n✯︙كل نقطه تساوي 50 رساله', 'md')
DevAbs:del(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_)
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
if Abs.members_[i].status_.ID == "ChatMemberStatusCreator" then  
Manager_id = admins[i].user_id_  
DevAbs:sadd(BANDE..'Abs:BasicConstructor:'..msg.chat_id_,Manager_id)  
DevAbs:sadd(BANDE..'Abs:AbsConstructor:'..msg.chat_id_,Manager_id)   
end  
end  
if num == 0 then
...
local msguser = tonumber(DevAbs:get(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_))
local user_msgs = DevAbs:get(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local cont = (tonumber(DevAbs:get(BANDE..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
local user_nkt = tonumber(DevAbs:get(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
if result.username_ then username = '@'..result.username_ else username = 'لا يوجد' end
...
DevAbs:setex(BANDE..'DevAbs4'..msg.sender_user_id_,360,true)
...
function Dev-BANDE(extra,result,success)
if DevAbs:get(BANDE.."Abs:Send:Bot"..BANDE) and not AbsSudo(msg) then 
send(msg.chat_id_, msg.id_,"✯︙الاذاعه معطله من قبل المطور الاساسي")
return false
end
local GpList = DevAbs:smembers(BANDE.."Abs:Groups")
for k,v in pairs(GpList) do
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = result.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end
local PvList = DevAbs:smembers(BANDE.."Abs:Users")
for k,v in pairs(PvList) do
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = result.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم اذاعة رسالتك بالتوجيه \n✯︙‏في ↫ ❨ '..#GpList..' ❩ مجموعه \n✯︙والى ↫ ❨ '..#PvList..' ❩ مشترك \n ✓', 1, 'md')
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),Dev-BANDE)
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == "مشاهده المنشور" and ChCheck(msg) or text == "مشاهدات المنشور" and ChCheck(msg) or text == "عدد المشاهدات" and ChCheck(msg) then
DevAbs:set(BANDE..'Abs:viewget'..msg.sender_user_id_,true)
...
if DevAbs:get(BANDE.."Abs:Kick:Me"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevAbs:set(BANDE..'yes'..msg.sender_user_id_, 'delyes')
DevAbs:set(BANDE..'no'..msg.sender_user_id_, 'delno')
...
DevAbs:set(BANDE.."Abs:Kick:Me"..msg.chat_id_, true)
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل امر اطردني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل اطردني' and Manager(msg) and ChCheck(msg) then
DevAbs:del(BANDE.."Abs:Kick:Me"..msg.chat_id_)
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل امر اطردني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == "نزلني" and ChCheck(msg) then
if DevAbs:get(BANDE.."Abs:Del:Me"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevAbs:set(BANDE..'yesdel'..msg.sender_user_id_, 'delyes')
DevAbs:set(BANDE..'nodel'..msg.sender_user_id_, 'delno')
...
DevAbs:set(BANDE.."Abs:Del:Me"..msg.chat_id_, true)
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل امر نزلني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل نزلني' and BasicConstructor(msg) and ChCheck(msg) then
DevAbs:del(BANDE.."Abs:Del:Me"..msg.chat_id_)
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل امر نزلني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:TagAll'..msg.chat_id_)
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:TagAll'..msg.chat_id_,true)
end
if Admin(msg) then
if text == "تاك للكل" and ChCheck(msg) then
if not DevAbs:get(BANDE..'Abs:Lock:TagAll'..msg.chat_id_) then
function TagAll(dp1,dp2)
local text = "✯︙وينكم يالربع \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
i = 0
for k, v in pairs(dp2.members_) do
i = i + 1
if DevAbs:get(BANDE..'Save:UserName'..v.user_id_) then
text = text..i.."~ : [@"..DevAbs:get(BANDE..'Save:UserName'..v.user_id_).."]\n"
else
...
local EntryNumber = (DevAbs:get(BANDE..'Abs:EntryNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local ExitNumber = (DevAbs:get(BANDE..'Abs:ExitNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local MsgNumberDay = (DevAbs:get(BANDE..'Abs:MsgNumberDay'..msg.chat_id_..':'..os.date('%d')) or 0)
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل تفاعلي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:msg:Abs'..msg.chat_id_) 
end
if text == "تفعيل تفاعلي" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل تفاعلي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:msg:Abs'..msg.chat_id_,true)  
end
if DevAbs:get(BANDE.."Abs:msg:Abs"..msg.chat_id_) then
if msg.content_.ID then
get_msg = DevAbs:get(BANDE.."Abs:msg:Abs"..msg.sender_user_id_..":"..msg.chat_id_) or 0
gms = get_msg + 1
DevAbs:setex(BANDE..'Abs:msg:Abs'..msg.sender_user_id_..":"..msg.chat_id_,86400,gms)
end
...
function prom_reply(extra, result, success) 
get_msg = DevAbs:get(BANDE.."Abs:msg:Abs"..result.sender_user_id_..":"..msg.chat_id_) or 0
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == "جهاتي" and ChCheck(msg) or text == "اضافاتي" and ChCheck(msg) then add = (tonumber(DevAbs:get(BANDE..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0) Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙عدد جهاتك المضافه ↫ *❨ "..add.." ❩* ", 1, 'md') end
if text == "تعديلاتي" or text == "سحكاتي" and ChCheck(msg) then local edit_msg = DevAbs:get(BANDE..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0  Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙عدد تعديلاتك ↫ *❨ "..edit_msg.." ❩* ", 1, 'md') end
...
if text == 'مسح جهاتي' or text == 'مسح اضافاتي' or text == 'حذف جهاتي' or text == 'حذف اضافاتي' then DevAbs:del(BANDE..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم حذف جميع جهاتك المضافه' , 1, 'md') end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text and text:match('^هينه @(.*)') and ChCheck(msg) or text and text:match('^هينها @(.*)') and ChCheck(msg) then 
if not DevAbs:get(BANDE..'Abs:Lock:Stupid'..msg.chat_id_) then
local username = text:match('^هينه @(.*)') or text:match('^هينها @(.*)') 
function Dev-BANDE(extra,result,success)
if result.id_ then  
if tonumber(result.id_) == tonumber(BANDE) then  
...
if DevAbs:sismember(BANDE.."Abs:AbsConstructor:"..msg.chat_id_,result.id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local Dev-BANDE = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md') 
local Dev-BANDE = { "لكك جرجف @"..username.." احترم اسيادكك لا اكتلكك وازربب على كبركك،💩🖐🏿","هشش لكك فاشل @"..username.." لتضل تمسلت لا اخربط تضاريس وجهك جنه ابط عبده، 😖👌🏿","حبيبي @"..username.." راح احاول احترمكك هالمره بلكي تبطل حيونه، 🤔🔪","دمشي لك @"..username.." ينبوع الفشل مو زين ملفيك ونحجي وياك هي منبوذ 😏🖕🏿","ها الغليض التفس ابو راس المربع @"..username.." متعلملك جم حجايه وجاي تطكطكهن علينه دبطل😒🔪",}
Dev_Abs(msg.chat_id_, result.id_, 1,''..Dev-BANDE[math.random(#Dev-BANDE)], 1, 'html') 
else  
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو غير موجود في المجموعه', 1, 'md') 
end 
end 
resolve_username(username,Dev-BANDE)
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
if tonumber(result.sender_user_id_) == tonumber(BANDE) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, 'شو تمضرط اكو واحد يهين نفسه؟🤔👌🏿', 1, 'md') 
return false  
end  
...
if DevAbs:sismember(BANDE.."Abs:AbsConstructor:"..msg.chat_id_,result.sender_user_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local Dev-BANDE = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md') 
local Dev-BANDE = {"لكك جرجف احترم اسيادكك لا اكتلكك وازربب على كبركك،💩🖐🏿","هشش فاشل لتضل تمسلت لا اخربط تضاريس وجهك جنه ابط عبده، 😖👌🏿","دمشي لك ينبوع الفشل مو زين ملفيك ونحجي وياك هي منبوذ 😏🖕🏿","ها الغليض التفس ابو راس المربع متعلملك جم حجايه وجاي تطكطكهن علينه دبطل😒🔪","حبيبي راح احاول احترمكك هالمره بلكي تبطل حيونه، 🤔🔪"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..Dev-BANDE[math.random(#Dev-BANDE)], 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
local Dev-BANDE = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md') 
local Dev-BANDE = {"مواححح افيش عافيههه😍🔥💗","امممووااهحح شهلعسل🥺🍯💘","مواححح،ءوفف اذوب🤤💗"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..Dev-BANDE[math.random(#Dev-BANDE)], 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
local Dev-BANDE = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md') 
local Dev-BANDE = {"تتعال ححب محتاجيك🙂🍭","تعال يولل استاذكك ايريدككك😒🔪","يمعوود تعاال يريدوكك🤕♥️","تعال لكك ديصيحوك😐🖤"} 
Dev_Abs(msg.chat_id_, result.id_, 1,''..Dev-BANDE[math.random(#Dev-BANDE)], 1, 'md') 
end 
if tonumber(msg.reply_to_message_id_) == 0 then
else 
...
local Dev-BANDE = "صارر ستاذيي 🏃🏻‍♂️♥️" 
Dev_Abs(msg.chat_id_, msg.id_, 1,Dev-BANDE, 1, 'md') 
local Dev-BANDE = { "تتعال ححب @"..username.." محتاجيك🙂🍭","تعال يولل @"..username.." استاذكك ايريدككك😒🔪","يمعوود @"..username.." تعاال يريدوكك🤕♥️","تعال لكك @"..username.." ديصيحوك😐🖤",}
Dev_Abs(msg.chat_id_, result.id_, 1,''..Dev-BANDE[math.random(#Dev-BANDE)], 1, 'html') 
else  
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙العضو غير موجود في المجموعه', 1, 'md') 
end 
...
if DevAbs:sismember(BANDE..'Abs:AbsSudo:',result.sender_user_id_) then
Abssudo = 'المطورين الاساسيين • ' else Abssudo = '' end
if DevAbs:sismember(BANDE..'Abs:SecondSudo:',result.sender_user_id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevAbs:sismember(BANDE..'Abs:SudoBot:',result.sender_user_id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevAbs:sismember(BANDE..'Abs:Owner:'..msg.chat_id_, result.sender_user_id_) then
owner = 'المالكين • ' else owner = '' end
if DevAbs:sismember(BANDE..'Abs:BasicConstructor:'..msg.chat_id_, result.sender_user_id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevAbs:sismember(BANDE..'Abs:Constructor:'..msg.chat_id_, result.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(BANDE..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_) then
manager = 'المدراء • ' else manager = '' end
if DevAbs:sismember(BANDE..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevAbs:sismember(BANDE..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(BANDE..'Abs:Cleaner:'..msg.chat_id_, result.sender_user_id_) then
...
DevAbs:srem(BANDE..'Abs:AbsSudo:', result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:SecondSudo:', result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:SudoBot:', result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:Owner:'..msg.chat_id_,result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:Constructor:'..msg.chat_id_,result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:Cleaner:'..msg.chat_id_, result.sender_user_id_)
...
if DevAbs:sismember(BANDE..'Abs:AbsSudo:',result.id_) then
Abssudo = 'المطورين الاساسيين • ' else Abssudo = '' end
if DevAbs:sismember(BANDE..'Abs:SecondSudo:',result.id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevAbs:sismember(BANDE..'Abs:SudoBot:',result.id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevAbs:sismember(BANDE..'Abs:Owner:'..msg.chat_id_, result.id_) then
owner = 'المالكين • ' else owner = '' end
if DevAbs:sismember(BANDE..'Abs:BasicConstructor:'..msg.chat_id_, result.id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevAbs:sismember(BANDE..'Abs:Constructor:'..msg.chat_id_, result.id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(BANDE..'Abs:Managers:'..msg.chat_id_, result.id_) then
manager = 'المدراء • ' else manager = '' end
if DevAbs:sismember(BANDE..'Abs:Admins:'..msg.chat_id_, result.id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevAbs:sismember(BANDE..'Abs:VipMem:'..msg.chat_id_, result.id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(BANDE..'Abs:Cleaner:'..msg.chat_id_, result.id_) then
cleaner = 'المنظفين • ' else cleaner = ''
end
...
if AbsDelAll(msg.sender_user_id_,msg.chat_id_) == 'sudoid' then
DevAbs:srem(BANDE..'Abs:AbsSudo:', result.id_)
DevAbs:srem(BANDE..'Abs:SecondSudo:', result.id_)
DevAbs:srem(BANDE..'Abs:SudoBot:', result.id_)
DevAbs:srem(BANDE..'Abs:Owner:'..msg.chat_id_,result.id_)
DevAbs:srem(BANDE..'Abs:BasicConstructor:'..msg.chat_id_,result.id_)
DevAbs:srem(BANDE..'Abs:Constructor:'..msg.chat_id_,result.id_)
DevAbs:srem(BANDE..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:srem(BANDE..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:srem(BANDE..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:srem(BANDE..'Abs:Cleaner:'..msg.chat_id_, result.id_)
elseif AbsDelAll(msg.sender_user_id_,msg.chat_id_) == 'Abssudo' then
DevAbs:srem(BANDE..'Abs:SecondSudo:', result.id_)
DevAbs:srem(BANDE..'Abs:SudoBot:', result.id_)
DevAbs:srem(BANDE..'Abs:Owner:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(BANDE..'Abs:AbsSudo:',result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(BANDE..'Abs:AbsSudo:',result.id_)
...
DevAbs:sadd(BANDE..'Abs:AbsSudo:',user)
...
DevAbs:srem(BANDE..'Abs:AbsSudo:',user)
...
function sudo_reply(extra, result, success)
DevAbs:sadd(BANDE..'Abs:SecondSudo:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(BANDE..'Abs:SecondSudo:',result.id_)
...
DevAbs:sadd(BANDE..'Abs:SecondSudo:',user)
...
DevAbs:srem(BANDE..'Abs:SecondSudo:',user)
...
function sudo_reply(extra, result, success)
DevAbs:sadd(BANDE..'Abs:SudoBot:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(BANDE..'Abs:SudoBot:',result.id_)
...
DevAbs:sadd(BANDE..'Abs:SudoBot:',user)
...
DevAbs:srem(BANDE..'Abs:SudoBot:',user)
...
DevAbs:sadd(BANDE..'Abs:Owner:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(BANDE..'Abs:Owner:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(BANDE..'Abs:Owner:'..msg.chat_id_,user)
...
DevAbs:srem(BANDE..'Abs:Owner:'..msg.chat_id_,user)
...
function raf_reply(extra, result, success)
if not AbsConstructor(msg) and DevAbs:get(BANDE.."Abs:Lock:ProSet"..msg.chat_id_) then 
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙لاتستطيع رفع احد وذالك بسبب تعطيل الرفع', 1, 'md')
return false
end
DevAbs:sadd(BANDE..'Abs:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(BANDE..'Abs:BasicConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(BANDE..'Abs:BasicConstructor:'..msg.chat_id_,user)
...
DevAbs:srem(BANDE..'Abs:BasicConstructor:'..msg.chat_id_,user)
...
DevAbs:sadd(BANDE..'Abs:Constructor:'..msg.chat_id_,result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(BANDE..'Abs:Constructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(BANDE..'Abs:Constructor:'..msg.chat_id_,user)
...
DevAbs:srem(BANDE..'Abs:Constructor:'..msg.chat_id_,user)
...
DevAbs:sadd(BANDE..'Abs:Managers:'..msg.chat_id_,user)
...
DevAbs:srem(BANDE..'Abs:Managers:'..msg.chat_id_,user)
...
DevAbs:sadd(BANDE..'Abs:Cleaner:'..msg.chat_id_,result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(BANDE..'Abs:Cleaner:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(BANDE..'Abs:Cleaner:'..msg.chat_id_,user)
...
DevAbs:srem(BANDE..'Abs:Cleaner:'..msg.chat_id_,user)
...
DevAbs:sadd(BANDE..'Abs:Admins:'..msg.chat_id_,user)
...
DevAbs:srem(BANDE..'Abs:Admins:'..msg.chat_id_,user)
...
DevAbs:sadd(BANDE..'Abs:VipMem:'..msg.chat_id_,user)
...
DevAbs:srem(BANDE..'Abs:VipMem:'..msg.chat_id_,user)
...
function promote_by_reply(extra, result, success)
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChatMember?chat_id='..msg.chat_id_..'&user_id='..BANDE)
local GetInfo = JSON.decode(Check)
...
Dev-BANDE = {"ها هلاو","انطق","كول","تفضل","احبك","عمري","لاف"};
send(msg.sender_user_id_, 0,Dev-BANDE[math.random(#Dev-BANDE)])
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
DevAbs:del(BANDE.."Abs:Lock:ProSet"..msg.chat_id_)
...
DevAbs:set(BANDE.."Abs:Lock:ProSet"..msg.chat_id_,"true")
...
if not AbsConstructor(msg) and DevAbs:get(BANDE.."Abs:Lock:KickBan"..msg.chat_id_) then 
...
ChatKick(result.chat_id_, result.sender_user_id_)
DevAbs:sadd(BANDE..'Abs:Ban:'..msg.chat_id_, result.sender_user_id_)
...
if not Constructor(msg) and DevAbs:get(BANDE.."Abs:Lock:KickBan"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_,'✯︙لقد تم تعطيل الطرد والحظر من قبل مالك المجموعه')
return false
end
if result.id_ then
...
ChatKick(msg.chat_id_, result.id_)
DevAbs:sadd(BANDE..'Abs:Ban:'..msg.chat_id_, result.id_)
...
ChatKick(msg.chat_id_, user)
DevAbs:sadd(BANDE..'Abs:Ban:'..msg.chat_id_, user)
...
DevAbs:srem(BANDE..'Abs:Ban:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
DevAbs:srem(BANDE..'Abs:Ban:'..msg.chat_id_, result.id_)
...
DevAbs:srem(BANDE..'Abs:Ban:'..msg.chat_id_, user)
...
if not AbsConstructor(msg) and DevAbs:get(BANDE.."Abs:Lock:MuteTked"..msg.chat_id_) then 
...
if DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","✯︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(BANDE..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","✯︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(BANDE..'Abs:Muted:'..msg.chat_id_, result.id_)
...
if DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","✯︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(BANDE..'Abs:Muted:'..msg.chat_id_, user)
...
if not DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","✯︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(BANDE..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
if not DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","✯︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(BANDE..'Abs:Muted:'..msg.chat_id_, result.id_)
...
if not DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","✯︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(BANDE..'Abs:Muted:'..msg.chat_id_, user)
...
DevAbs:sadd(BANDE..'Abs:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:sadd(BANDE..'Abs:Tkeed:'..msg.chat_id_, result.id_)
...
DevAbs:sadd(BANDE..'Abs:Tkeed:'..msg.chat_id_, user)
...
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_, result.id_)
...
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_, user)
...
if DevAbs:sismember(BANDE..'Abs:AbsSudo:',result.sender_user_id_) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙*لاتستطيع حظر المطور الاساسي*", 1, 'md')
return false 
end
if DevAbs:sismember(BANDE..'Abs:SecondSudo:',result.sender_user_id_) and not AbsSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙*لاتستطيع حظر المطور الاساسي²*", 1, 'md')
return false 
end
ChatKick(result.chat_id_, result.sender_user_id_)
DevAbs:sadd(BANDE..'Abs:BanAll:', result.sender_user_id_)
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
if DevAbs:sismember(BANDE..'Abs:AbsSudo:',result.id_) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙*لاتستطيع حظر المطور الاساسي*", 1, 'md')
return false 
end
if DevAbs:sismember(BANDE..'Abs:SecondSudo:',result.id_) and not AbsSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙*لاتستطيع حظر المطور الاساسي²*", 1, 'md')
return false 
end
if result.id_ then
ChatKick(msg.chat_id_, result.id_)
DevAbs:sadd(BANDE..'Abs:BanAll:', result.id_)
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
if tonumber(user) == tonumber(BANDE) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙*لاتستطيع حظر البوت عام*", 1, 'md')
return false 
end
...
if DevAbs:sismember(BANDE..'Abs:AbsSudo:',user) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙*لاتستطيع حظر المطور الاساسي*", 1, 'md')
return false 
end
if DevAbs:sismember(BANDE..'Abs:SecondSudo:',user) and not AbsSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙*لاتستطيع حظر المطور الاساسي²*", 1, 'md')
return false 
end
ChatKick(msg.chat_id_, user)
DevAbs:sadd(BANDE..'Abs:BanAll:', user)
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
DevAbs:sadd(BANDE..'Abs:MuteAll:', result.sender_user_id_)  
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
if result.id_ then
DevAbs:sadd(BANDE..'Abs:MuteAll:', result.id_)
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
DevAbs:sadd(BANDE..'Abs:MuteAll:', user)
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
DevAbs:srem(BANDE..'Abs:BanAll:', result.sender_user_id_)
DevAbs:srem(BANDE..'Abs:MuteAll:', result.sender_user_id_)
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
if result.id_ then
DevAbs:srem(BANDE..'Abs:BanAll:', result.id_)
DevAbs:srem(BANDE..'Abs:MuteAll:', result.id_)
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
DevAbs:srem(BANDE..'Abs:BanAll:', user)
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
DevAbs:srem(BANDE..'Abs:MuteAll:', user)
local gps = DevAbs:scard(BANDE.."Abs:Groups")
...
DevAbs:setex(BANDE.."Abs:EditDev"..msg.sender_user_id_,300,true)
end
if DevAbs:get(BANDE.."Abs:EditDev"..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_,'✯︙تم الغاء امر تغير المطور الاساسي')
DevAbs:del(BANDE.."Abs:EditDev"..msg.sender_user_id_)
return false
end
if text and text:match("^(%d+)$") then 
...
if dp.first_name_ ~= false then
DevAbs:del(BANDE.."Abs:EditDev"..msg.sender_user_id_)
DevAbs:set(BANDE.."Abs:NewDev"..msg.sender_user_id_,dp.id_)
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if msg.reply_to_message_id_ ~= 0 then
if text and text:match("^رفع مطي$") and not DevAbs:get(BANDE..'Abs:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if DevAbs:sismember(BANDE..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","✯︙هو مطي شرفع منه بعد😹💔") 
else
ReplyStatus(msg,result.sender_user_id_,"Reply","✯︙تم رفعه في قائمة المطايه") 
DevAbs:sadd(BANDE..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
end end
getMessage(msg.chat_id_, msg.reply_to_message_id_,donky_by_reply)
end end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if msg.reply_to_message_id_ ~= 0  then
if text and text:match("^تنزيل مطي$") and not DevAbs:get(BANDE..'Abs:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if not DevAbs:sismember(BANDE..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","✯︙هو ليس مطي ليتم تنزيله") 
else
DevAbs:srem(BANDE..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:set('Dev-BANDE:'..BANDE..'id:user'..msg.chat_id_,TXT)  
DevAbs:setex('Dev-BANDE:'..BANDE.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
DevAbs:del(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_) 
DevAbs:incrby(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_,Num) 
...
DevAbs:set('Dev-BANDE:'..BANDE..'ids:user'..msg.chat_id_,TXT)  
DevAbs:setex('Dev-BANDE:'..BANDE.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
function Reply(extra, result, success)
DevAbs:incrby(BANDE..'Abs:GamesNumber'..msg.chat_id_..result.sender_user_id_,Num) 
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم اضافة "..Num..' نقطه', 1, 'md') 
end
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},Reply, nil)
return false
end
if DevAbs:get(BANDE..'Abs:Lock:Clean'..msg.chat_id_) then if msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.photo_ or msg.content_.animation_ or msg.content_.animated_ then if msg.reply_to_message_id_ ~= 0 then DevAbs:sadd(BANDE.."Abs:cleaner"..msg.chat_id_, msg.id_) else DevAbs:sadd(BANDE.."Abs:cleaner"..msg.chat_id_, msg.id_) end end end
if DevAbs:get(BANDE..'Abs:Lock:CleanNum'..msg.chat_id_) then if msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.photo_ or msg.content_.animation_ or msg.content_.animated_ then if msg.reply_to_message_id_ ~= 0 then DevAbs:sadd(BANDE.."Abs:cleanernum"..msg.chat_id_, msg.id_) else DevAbs:sadd(BANDE.."Abs:cleanernum"..msg.chat_id_, msg.id_) end end end
if DevAbs:get(BANDE..'Abs:Lock:CleanMusic'..msg.chat_id_) then if msg.content_.voice_ or msg.content_.audio_ then if msg.reply_to_message_id_ ~= 0 then DevAbs:sadd(BANDE.."Abs:cleanermusic"..msg.chat_id_, msg.id_) else DevAbs:sadd(BANDE.."Abs:cleanermusic"..msg.chat_id_, msg.id_) end end end
if Manager(msg) and msg.reply_to_message_id_ ~= 0 then
if text and text:match("^تثبيت$") and ChCheck(msg) then 
if DevAbs:sismember(BANDE.."Abs:Lock:Pinpin",msg.chat_id_) and not BasicConstructor(msg) then
...
if data.ID == "Ok" then
DevAbs:set(BANDE..'Abs:PinnedMsg'..msg.chat_id_,msg.reply_to_message_id_)
...
local List = DevAbs:smembers(BANDE..'Abs:VipMem:'..msg.chat_id_)
text = "✯︙قائمة المميزين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(BANDE..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(BANDE..Abs)
text = "✯︙قائمة الادمنيه ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(BANDE..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(BANDE..'Abs:Managers:'..msg.chat_id_)
text = "✯︙قائمة المدراء ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(BANDE..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(BANDE..'Abs:Cleaner:'..msg.chat_id_)
text = "✯︙قائمة المنظفين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(BANDE..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(BANDE..'Abs:Constructor:'..msg.chat_id_)
text = "✯︙قائمة المنشئين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(BANDE..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(BANDE..'Abs:Owner:'..msg.chat_id_)
text = "✯︙قائمة المالكين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(BANDE..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(BANDE..'Abs:BasicConstructor:'..msg.chat_id_)
...
keyboard = {} 
keyboard.inline_keyboard = {{{text=''..dp.first_name_..'',url="t.me/"..dp.username_ or Dev-BANDE}}}
local msg_id = msg.id_/2097152/0.5
https.request("https://api.telegram.org/bot"..TokenBot..'/sendPhoto?chat_id='..msg.chat_id_..'&caption='..URL.escape(Text)..'&photo='..getpro.photos_[0].sizes_[1].photo_.persistent_id_..'&reply_to_message_id='..msg_id..'&parse_mode=markdown&disable_web_page_preview=true&reply_markup='..JSON.encode(keyboard)) 
else
...
local List = DevAbs:smembers(BANDE..'Abs:Muted:'..msg.chat_id_)
...
local List = DevAbs:smembers(BANDE..'Abs:Tkeed:'..msg.chat_id_)
text = "✯︙قائمة المقيدين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(BANDE..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(BANDE..'Abs:Ban:'..msg.chat_id_)
...
local List = DevAbs:smembers(BANDE..'User:Donky:'..msg.chat_id_)
...
local List = DevAbs:hkeys(BANDE..'Abs:Filters:'..msg.chat_id_)
text = "✯︙قائمة المنع ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k, v in pairs(List) do
text = text..k..'~ ❨ '..v..' ❩\n'
end
...
local List = DevAbs:smembers(BANDE..'Abs:AbsSudo:')
...
local List = DevAbs:smembers(BANDE..'Abs:SecondSudo:')
...
local BanAll = DevAbs:smembers(BANDE..'Abs:BanAll:')
local MuteAll = DevAbs:smembers(BANDE..'Abs:MuteAll:')
if #BanAll ~= 0 then 
...
local List = DevAbs:smembers(BANDE..'Abs:SudoBot:')
text = "✯︙قائمة المطورين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local sudouser = DevAbs:get(BANDE..'Abs:Sudos'..v) 
local username = DevAbs:get(BANDE..'Save:UserName'..v)
if username then
...
DevAbs:sadd(BANDE.."Abs:AbsConstructor:"..msg.chat_id_,dp.id_)
end,nil)   
end,nil)   
end
end 
...
if result.content_.sticker_ then
local idsticker = result.content_.sticker_.sticker_.persistent_id_
DevAbs:srem(BANDE.."Abs:FilterSteckr"..msg.chat_id_,idsticker)
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء منع الملصق يمكنهم ارساله الان', 1, 'md')
return false
end
if result.content_.ID == "MessagePhoto" then
local photo = result.content_.photo_.id_
DevAbs:srem(BANDE.."Abs:FilterPhoto"..msg.chat_id_,photo)
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء منع الصوره يمكنهم ارسالها الان', 1, 'md')
return false
end
if result.content_.animation_.animation_ then
local idanimation = result.content_.animation_.animation_.persistent_id_
DevAbs:srem(BANDE.."Abs:FilterAnimation"..msg.chat_id_,idanimation)
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Thwel:Abs'..msg.chat_id_) 
end
if text and (text == "تعطيل تحويل الصيغ" or text == "تعطيل التحويل") and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل تحويل الصيغ'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Thwel:Abs'..msg.chat_id_,true)  
end
if text == 'تحويل' and not DevAbs:get(BANDE..'Abs:Thwel:Abs'..msg.chat_id_) then  
if tonumber(msg.reply_to_message_id_) > 0 then 
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local user_msgs = DevAbs:get(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..data.id_) or 0
local user_nkt = tonumber(DevAbs:get(BANDE..'Abs:GamesNumber'..msg.chat_id_..data.id_) or 0)
if DevAbs:sismember(BANDE..'Abs:BanAll:',result.sender_user_id_) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(BANDE..'Abs:MuteAll:',result.sender_user_id_) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(BANDE..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'محظور'
elseif DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(BANDE..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(BANDE..'Abs:SudoBot:',result.sender_user_id_) and SudoBot(msg) then
sudobot = '\n✯︙عدد الكروبات ↫ '..(DevAbs:get(BANDE..'Abs:Sudos'..result.sender_user_id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevAbs:get(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..res.id_) or 0
local user_nkt = tonumber(DevAbs:get(BANDE..'Abs:GamesNumber'..msg.chat_id_..res.id_) or 0)
if DevAbs:sismember(BANDE..'Abs:BanAll:',res.id_) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(BANDE..'Abs:MuteAll:',res.id_) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(BANDE..'Abs:Ban:'..msg.chat_id_,res.id_) then
Tkeed = 'محظور'
elseif DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_,res.id_) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(BANDE..'Abs:Tkeed:'..msg.chat_id_,res.id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(BANDE..'Abs:SudoBot:',res.id_) and SudoBot(msg) then
sudobot = '\n✯︙عدد الكروبات ↫ '..(DevAbs:get(BANDE..'Abs:Sudos'..res.id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevAbs:get(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..iduser) or 0
local user_nkt = tonumber(DevAbs:get(BANDE..'Abs:GamesNumber'..msg.chat_id_..iduser) or 0)
if DevAbs:sismember(BANDE..'Abs:BanAll:',iduser) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(BANDE..'Abs:MuteAll:',iduser) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(BANDE..'Abs:Ban:'..msg.chat_id_,iduser) then
Tkeed = 'محظور'
elseif DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_,iduser) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(BANDE..'Abs:Tkeed:'..msg.chat_id_,iduser) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(BANDE..'Abs:SudoBot:',iduser) and SudoBot(msg) then
sudobot = '\n✯︙عدد الكروبات ↫ '..(DevAbs:get(BANDE..'Abs:Sudos'..iduser) or 0)..'' 
else 
sudobot = '' 
end
...
if DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevAbs:sismember(BANDE..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevAbs:sismember(BANDE..'Abs:BanAll:',result.sender_user_id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevAbs:sismember(BANDE..'Abs:MuteAll:',result.sender_user_id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevAbs:sismember(BANDE..'Abs:Tkeed:',result.sender_user_id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
...
if result.id_ then
if DevAbs:sismember(BANDE..'Abs:Muted:'..msg.chat_id_,result.id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevAbs:sismember(BANDE..'Abs:Ban:'..msg.chat_id_,result.id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevAbs:sismember(BANDE..'Abs:BanAll:',result.id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevAbs:sismember(BANDE..'Abs:MuteAll:',result.id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevAbs:sismember(BANDE..'Abs:Tkeed:',result.id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙الحظر العام ↫ '..banall..'\n✯︙الكتم العام ↫ '..muteall..'\n✯︙الحظر ↫ '..banned..'\n✯︙الكتم ↫ '..muted..'\n✯︙التقيد ↫ '..tkeed, 1, 'md')  
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙*المعرف غير صحيح*', 1, 'md')  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(BANDE..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(BANDE..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(BANDE..'Abs:BanAll:',result.sender_user_id_) DevAbs:srem(BANDE..'Abs:MuteAll:',result.sender_user_id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(BANDE..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(BANDE..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) 
end
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),unbanreply) 
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_,user) DevAbs:srem(BANDE..'Abs:Ban:'..msg.chat_id_,user) DevAbs:srem(BANDE..'Abs:Muted:'..msg.chat_id_,user) DevAbs:srem(BANDE..'Abs:BanAll:',user) DevAbs:srem(BANDE..'Abs:MuteAll:',user)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_,user) DevAbs:srem(BANDE..'Abs:Ban:'..msg.chat_id_,user) DevAbs:srem(BANDE..'Abs:Muted:'..msg.chat_id_,user) 
end  
end,nil)  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_,result.id_) DevAbs:srem(BANDE..'Abs:Ban:'..msg.chat_id_,result.id_) DevAbs:srem(BANDE..'Abs:Muted:'..msg.chat_id_,result.id_) DevAbs:srem(BANDE..'Abs:BanAll:',result.id_) DevAbs:srem(BANDE..'Abs:MuteAll:',result.id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_,result.id_) DevAbs:srem(BANDE..'Abs:Ban:'..msg.chat_id_,result.id_) DevAbs:srem(BANDE..'Abs:Muted:'..msg.chat_id_,result.id_) 
end
end,nil)   
end  
...
DevAbs:set(BANDE.."Abs:GpIds:Text"..msg.chat_id_,Text_Rand)
...
DevAbs:set("BANDE:New:id:"..BANDE..msg.sender_user_id_,'Dev-BANDE')
return "Dev-BANDE"
end
if text and DevAbs:get("BANDE:New:id:"..BANDE..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevAbs:del("BANDE:New:id:"..BANDE..msg.sender_user_id_)
return false
end
DevAbs:del("BANDE:New:id:"..BANDE..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم حفظ كليشة الايدي العامه', 1, 'md')
DevAbs:set(BANDE.."Abs:AllIds:Text",text)
return false
end
...
DevAbs:del(BANDE.."Abs:AllIds:Text")
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text and text:match("^تعيين الايدي$") and ChCheck(msg) or text and text:match("^تعين الايدي$") and ChCheck(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙رجائا اتبع التعليمات للتعيين \n✯︙لطبع كليشة الايدي ارسل كليشه تحتوي على النصوص التي باللغه الانجليزيه ادناه ↫ ⤈\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n `#username` ↬ لطبع المعرف\n `#id` ↬ لطبع الايدي \n `#photos` ↬ لطبع عدد الصور \n `#stast` ↬ لطبع الرتب \n `#msgs` ↬ لطبع عدد الرسائل \n `#msgday` ↬ لطبع الرسائل اليوميه \n `#CustomTitle` ↬ لطبع اللقب \n `#bio` ↬ لطبع البايو \n `#auto` ↬ لطبع التفاعل \n `#game` ↬ لطبع عدد النقاط \n `#cont` ↬ لطبع عدد الجهات \n `#edit` ↬ لطبع عدد السحكات \n `#Description` ↬ لطبع تعليق الصور\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉', 1, 'md')
DevAbs:set("BANDE:New:id:"..BANDE..msg.chat_id_..msg.sender_user_id_,'Dev-BANDE')
return "Dev-BANDE"
end
if text and Manager(msg) and DevAbs:get("BANDE:New:id:"..BANDE..msg.chat_id_..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevAbs:del("BANDE:New:id:"..BANDE..msg.chat_id_..msg.sender_user_id_)
return false
end
DevAbs:del("BANDE:New:id:"..BANDE..msg.chat_id_..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم حفظ الكليشه الجديده', 1, 'md')
DevAbs:set(BANDE.."Abs:GpIds:Text"..msg.chat_id_,text)
return false
end
if text and text:match("^حذف الايدي$") and ChCheck(msg) or text and text:match("^مسح الايدي$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم حذف كليشة الايدي")  
DevAbs:del(BANDE.."Abs:GpIds:Text"..msg.chat_id_)
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if msg.reply_to_message_id_ ~= 0 then
...
function Dev-BANDE(extra,Abs,success)
...
local msgsday = DevAbs:get(BANDE..'Abs:UsersMsgs'..BANDE..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit_msg = DevAbs:get(BANDE..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(BANDE..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local user_nkt = tonumber(DevAbs:get(BANDE..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
local cont = (tonumber(DevAbs:get(BANDE..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
...
if not DevAbs:get(BANDE..'Abs:Lock:Id'..msg.chat_id_) then 
if not DevAbs:get(BANDE..'Abs:Lock:Id:Photo'..msg.chat_id_) then 
if DevAbs:get(BANDE.."Abs:AllIds:Text") then
newpicid = DevAbs:get(BANDE.."Abs:AllIds:Text")
...
if not DevAbs:get(BANDE.."Abs:GpIds:Text"..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, Abs.photos_[0].sizes_[1].photo_.persistent_id_,newpicid,msg.id_,msg.id_.."")
else 
local new_id = DevAbs:get(BANDE.."Abs:GpIds:Text"..msg.chat_id_)
...
if DevAbs:get(BANDE.."Abs:AllIds:Text") then
newallid = DevAbs:get(BANDE.."Abs:AllIds:Text")
...
if DevAbs:get(BANDE.."Abs:AllIds:Text") then
notpicid = DevAbs:get(BANDE.."Abs:AllIds:Text")
...
if not DevAbs:get(BANDE..'Abs:Lock:EditMsgs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم قفل التعديل")  
DevAbs:set(BANDE..'Abs:Lock:EditMsgs'..msg.chat_id_,true)
else
...
DevAbs:set(BANDE.."Abs:Lock:Bots"..msg.chat_id_,"ked")  
end
if LockText[2] == "التكرار" or LockText[2] == "التكرار بالحذف" then 
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","del")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم قفل التكرار بالحذف")  
end
if LockText[2] == "التكرار بالطرد" then 
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","kick")  
...
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم قفل التكرار بالتقيد")  
end
if LockText[2] == "التكرار بالكتم" then 
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","mute")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم قفل التكرار بالكتم")  
end
if BasicConstructor(msg) then
if LockText[2] == "التثبيت" then
if not DevAbs:get(BANDE..'Abs:Lock:Pin'..msg.chat_id_) then
tdcli_function ({ ID = "GetChannelFull",  channel_id_ = msg.chat_id_:gsub("-100","") }, function(arg,data)  DevAbs:set(BANDE.."Abs:PinnedMsg"..msg.chat_id_,data.pinned_message_id_)  end,nil)
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم قفل التثبيت")  
DevAbs:set(BANDE..'Abs:Lock:Pin'..msg.chat_id_,true)
DevAbs:sadd(BANDE.."Abs:Lock:Pinpin",msg.chat_id_) 
else
...
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Num:Spam" ,TextSpam) 
end
end
if text and (text:match("^ضع زمن التكرار (%d+)$") or text:match("^وضع زمن التكرار (%d+)$")) then  
local TextSpam = text:match("ضع زمن التكرار (%d+)$") or text:match("وضع زمن التكرار (%d+)$")
DevAbs:hset(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Num:Spam:Time" ,TextSpam) 
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:Id:Photo'..msg.chat_id_)
end end
if text and text == 'تعطيل الايدي بالصوره' and ChCheck(msg) then
if DevAbs:get(BANDE..'Abs:Lock:Id:Photo'..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙الايدي بالصوره بالتاكيد معطل', 1, 'md')
else
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الايدي بالصوره'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:Id:Photo'..msg.chat_id_,true)
end end 
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:Id'..msg.chat_id_)
end end 
if text and text == 'تعطيل الايدي' and ChCheck(msg) then
if DevAbs:get(BANDE..'Abs:Lock:Id'..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙الايدي بالتاكيد معطل ', 1, 'md')
else
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الايدي بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:Id'..msg.chat_id_,true)
end end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
DevAbs:setex(BANDE.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_,300,true) 
end
if text == 'انشاء رابط' and ChCheck(msg) or text == 'انشاء الرابط' and ChCheck(msg) then
local LinkGp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..msg.chat_id_))
if not DevAbs:get(BANDE.."Abs:Groups:Links"..msg.chat_id_)  then 
if LinkGp.ok == true then 
LinkGroup = LinkGp.result
DevAbs:set(BANDE.."Abs:Groups:Links"..msg.chat_id_,LinkGroup) 
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE.."Abs:Lock:Welcome"..msg.chat_id_,true)
end
if text and text:match("^تعطيل الترحيب$") and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الترحيب بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE.."Abs:Lock:Welcome"..msg.chat_id_)
end
if DevAbs:get(BANDE..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء حفظ كليشة الترحيب', 1, 'md')
DevAbs:del(BANDE..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(BANDE..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
Welcomes = text:gsub('"',"") Welcomes = text:gsub("'","") Welcomes = text:gsub(",","") Welcomes = text:gsub("*","") Welcomes = text:gsub(";","") Welcomes = text:gsub("`","") Welcomes = text:gsub("{","") Welcomes = text:gsub("}","") 
DevAbs:set(BANDE..'Abs:Groups:Welcomes'..msg.chat_id_,Welcomes)
...
DevAbs:set(BANDE..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
if text and text:match("^حذف الترحيب$") and ChCheck(msg) or text and text:match("^حذف ترحيب$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم حذف الترحيب")  
DevAbs:del(BANDE..'Abs:Groups:Welcomes'..msg.chat_id_)
end
if text and text:match("^جلب الترحيب$") and ChCheck(msg) or text and text:match("^جلب ترحيب$") and ChCheck(msg) or text and text:match("^الترحيب$") and ChCheck(msg) then
local Welcomes = DevAbs:get(BANDE..'Abs:Groups:Welcomes'..msg.chat_id_)
if Welcomes then
Dev_Abs(msg.chat_id_, msg.id_, 1, Welcomes, 1, 'md')
else
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_) then  
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء حفظ الوصف", 1, 'md')
DevAbs:del(BANDE..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(BANDE..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevAbs:set(BANDE..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
DevAbs:set(BANDE.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_,"add")  
return false  
end    
if DevAbs:get(BANDE.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "add" then
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر المنع', 1, 'md')
DevAbs:del(BANDE.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم منع الكلمه ↫ "..text, 1, 'html')
DevAbs:del(BANDE.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
DevAbs:hset(BANDE..'Abs:Filters:'..msg.chat_id_, text,'newword')
return false
end
if text and text == "الغاء منع" and msg.reply_to_message_id_ == 0 and ChCheck(msg) then       
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙ارسل لي الكلمه الان", 1, 'md') 
DevAbs:set(BANDE.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_,"del")  
return false  
end    
if DevAbs:get(BANDE.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "del" then   
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر الغاء المنع', 1, 'md')
DevAbs:del(BANDE.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
if not DevAbs:hget(BANDE..'Abs:Filters:'..msg.chat_id_, text) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙الكلمه ↫ "..text.." غير ممنوعه", 1, 'html')
DevAbs:del(BANDE.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
else
DevAbs:hdel(BANDE..'Abs:Filters:'..msg.chat_id_, text)
...
local gps = DevAbs:scard(BANDE.."Abs:Groups") local users = DevAbs:scard(BANDE.."Abs:Users") 
...
local users = DevAbs:scard(BANDE.."Abs:Users")
...
if not DevAbs:get(BANDE..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_) then  
...
DevAbs:setex(BANDE..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_,300,true)
end 
end
...
local pv = DevAbs:smembers(BANDE.."Abs:Users")
local sendok = 0
for i = 1, #pv do
...
DevAbs:srem(BANDE.."Abs:Users",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = DevAbs:smembers(BANDE.."Abs:Groups")
local w = 0
local q = 0
for i = 1, #group do
tdcli_function({ID='GetChat',chat_id_ = group[i]},function(arg,data)
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
DevAbs:srem(BANDE.."Abs:Groups",group[i]) 
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = group[i], user_id_ = BANDE, status_ = { ID = "ChatMemberStatusLeft" }, }, dl_cb, nil)
w = w + 1
end
...
Dev_Abs(msg.chat_id_, msg.id_, 1,'✯︙عدد الكروبات الان ↫ { '..#group..' }'..BANDEgp1..BANDEgp2..'\n✯︙العدد الحقيقي الان  ↫ ( '..BANDEgp3..' ) \n ', 1, 'md')
end end
end,nil)
end
return false
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:CleanNum'..msg.chat_id_,true)  
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:CleanNum'..msg.chat_id_) 
end
...
DevAbs:set(BANDE..'Abs:CleanNum'..msg.chat_id_,Num) 
end end 
if msg and DevAbs:get(BANDE..'Abs:Lock:CleanNum'..msg.chat_id_) then
if DevAbs:get(BANDE..'Abs:CleanNum'..msg.chat_id_) then CleanNum = DevAbs:get(BANDE..'Abs:CleanNum'..msg.chat_id_) else CleanNum = 200 end
if DevAbs:scard(BANDE.."Abs:cleanernum"..msg.chat_id_) >= tonumber(CleanNum) then 
local List = DevAbs:smembers(BANDE.."Abs:cleanernum"..msg.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
local Message = v
...
DevAbs:del(BANDE.."Abs:cleanernum"..msg.chat_id_)
end 
end
if CleanerNum(msg) then
if DevAbs:get(BANDE..'Abs:Lock:CleanNum'..msg.chat_id_) then 
if text == "التلقائي" and ChCheck(msg) or text == "عدد التلقائي" and ChCheck(msg) then 
local M = DevAbs:scard(BANDE.."Abs:cleanernum"..msg.chat_id_)
if M ~= 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙عدد الميديا ↫ "..M.."\n✯︙الحذف التلقائي ↫ "..(DevAbs:get(BANDE..'Abs:CleanNum'..msg.chat_id_) or 200), 1, 'md') 
else
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل امسح بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:Clean'..msg.chat_id_,true)  
end
if text == "تعطيل امسح" and Constructor(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل امسح بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:Clean'..msg.chat_id_) 
end
if Cleaner(msg) then
if DevAbs:get(BANDE..'Abs:Lock:Clean'..msg.chat_id_) then 
if text == "الميديا" and ChCheck(msg) or text == "عدد الميديا" and ChCheck(msg) then 
local M = DevAbs:scard(BANDE.."Abs:cleaner"..msg.chat_id_)
if M ~= 0 then
...
DevAbs:del(BANDE.."Abs:cleaner"..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙لاتوجد ميديا هنا", 1, 'md') 
end end 
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:CleanMusic'..msg.chat_id_,true)  
end
if text == "تعطيل مسح الاغاني" and Constructor(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل مسح الاغاني بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:CleanMusic'..msg.chat_id_) 
end
if CleanerMusic(msg) then
if DevAbs:get(BANDE..'Abs:Lock:CleanMusic'..msg.chat_id_) then 
if text == "الاغاني" and ChCheck(msg) or text == "عدد الاغاني" and ChCheck(msg) then 
local M = DevAbs:scard(BANDE.."Abs:cleanermusic"..msg.chat_id_)
if M ~= 0 then
...
local List = DevAbs:smembers(BANDE.."Abs:cleanermusic"..msg.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
local Message = v
...
DevAbs:del(BANDE.."Abs:cleanermusic"..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙لاتوجد اغاني هنا", 1, 'md') 
end end end end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
if DevAbs:get(BANDE..'Abs:Lock:EditMsgs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم فتح التعديل")  
DevAbs:del(BANDE..'Abs:Lock:EditMsgs'..msg.chat_id_)
else
...
DevAbs:del(BANDE.."Abs:Lock:Bots"..msg.chat_id_)  
end end
if UnLockText[2] == "التكرار" then 
DevAbs:hdel(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم فتح التكرار")  
end
if BasicConstructor(msg) then
if UnLockText[2] == "التثبيت" then
if DevAbs:get(BANDE..'Abs:Lock:Pin'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم فتح التثبيت")  
DevAbs:del(BANDE..'Abs:Lock:Pin'..msg.chat_id_)
DevAbs:srem(BANDE.."Abs:Lock:Pinpin",msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Text'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Photo'..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙الصور بالفعل مفتوحه في المجموعه', 1, 'md')
end
end
if UnLockText[2] == "الكلايش" then
if DevAbs:get(BANDE..'Abs:Lock:Spam'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم فتح الكلايش")  
DevAbs:del(BANDE..'Abs:Lock:Spam'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Join'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Videos'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Document'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Inline'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Markdown'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Gifs'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Music'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Voice'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Links'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Location'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Tags'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Hashtak'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Contact'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:WebLinks'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Arabic'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:English'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:TagServr'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Stickers'..msg.chat_id_)
else
...
DevAbs:del(BANDE..'Abs:Lock:Forwards'..msg.chat_id_)
else
...
DevAbs:set(BANDE.."Abs:Lock:Bots"..msg.chat_id_,"del") DevAbs:hset(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
LockList ={'Abs:Lock:Links','Abs:Lock:Contact','Abs:Lock:Forwards','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:EditMsgs','Abs:Lock:Stickers','Abs:Lock:Farsi','Abs:Lock:Spam','Abs:Lock:WebLinks','Abs:Lock:Photo'}
for i,Lock in pairs(LockList) do
DevAbs:set(BANDE..Lock..msg.chat_id_,true)
end
...
DevAbs:del(BANDE..UnLock..msg.chat_id_)
end
...
DevAbs:del(BANDE..'Abs:Lock:Fshar'..msg.chat_id_) DevAbs:del(BANDE..'Abs:Lock:Taf'..msg.chat_id_) DevAbs:del(BANDE..'Abs:Lock:Kfr'..msg.chat_id_) 
DevAbs:set(BANDE.."Abs:Lock:Bots"..msg.chat_id_,"del") DevAbs:hset(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
LockList ={'Abs:Lock:EditMsgs','Abs:Lock:Farsi','Abs:Lock:TagServr','Abs:Lock:Inline','Abs:Lock:Photo','Abs:Lock:Spam','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:Music','Abs:Lock:Voice','Abs:Lock:Links','Abs:Lock:Location','Abs:Lock:Tags','Abs:Lock:Stickers','Abs:Lock:Markdown','Abs:Lock:Forwards','Abs:Lock:Document','Abs:Lock:Contact','Abs:Lock:Hashtak','Abs:Lock:WebLinks'}
for i,Lock in pairs(LockList) do
DevAbs:set(BANDE..Lock..msg.chat_id_,true)
...
DevAbs:set(BANDE..'Abs:Lock:Fshar'..msg.chat_id_,true) DevAbs:set(BANDE..'Abs:Lock:Taf'..msg.chat_id_,true) DevAbs:set(BANDE..'Abs:Lock:Kfr'..msg.chat_id_,true) DevAbs:hdel(BANDE.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User") 
UnLockList ={'Abs:Lock:EditMsgs','Abs:Lock:Text','Abs:Lock:Arabic','Abs:Lock:English','Abs:Lock:Join','Abs:Lock:Bots','Abs:Lock:Farsi','Abs:Lock:FarsiBan','Abs:Lock:TagServr','Abs:Lock:Inline','Abs:Lock:Photo','Abs:Lock:Spam','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:Music','Abs:Lock:Voice','Abs:Lock:Links','Abs:Lock:Location','Abs:Lock:Tags','Abs:Lock:Stickers','Abs:Lock:Markdown','Abs:Lock:Forwards','Abs:Lock:Document','Abs:Lock:Contact','Abs:Lock:Hashtak','Abs:Lock:WebLinks'}
for i,UnLock in pairs(UnLockList) do
DevAbs:del(BANDE..UnLock..msg.chat_id_)
end
...
DevAbs:set(BANDE..'Abs:Spam:Text'..msg.chat_id_,SetSpam)
...
DevAbs:set(BANDE.."Abs:SudoBot:Rd"..msg.chat_id_,Text)
...
DevAbs:set(BANDE.."Abs:BasicConstructor:Rd"..msg.chat_id_,Text)
...
DevAbs:set(BANDE.."Abs:Constructor:Rd"..msg.chat_id_,Text)
...
DevAbs:set(BANDE.."Abs:Managers:Rd"..msg.chat_id_,Text) 
...
DevAbs:set(BANDE.."Abs:Admins:Rd"..msg.chat_id_,Text)
...
DevAbs:set(BANDE.."Abs:VipMem:Rd"..msg.chat_id_,Text)
...
DevAbs:set(BANDE.."Abs:Cleaner:Rd"..msg.chat_id_,Text)
...
DevAbs:set(BANDE.."Abs:mem:Rd"..msg.chat_id_,Text)
...
DevAbs:del(BANDE.."Abs:mem:Rd"..msg.chat_id_)
DevAbs:del(BANDE.."Abs:Cleaner:Rd"..msg.chat_id_)
DevAbs:del(BANDE.."Abs:VipMem:Rd"..msg.chat_id_)
DevAbs:del(BANDE.."Abs:Admins:Rd"..msg.chat_id_)
DevAbs:del(BANDE.."Abs:Managers:Rd"..msg.chat_id_)
DevAbs:del(BANDE.."Abs:Constructor:Rd"..msg.chat_id_)
DevAbs:del(BANDE.."Abs:BasicConstructor:Rd"..msg.chat_id_)
DevAbs:del(BANDE.."Abs:SudoBot:Rd"..msg.chat_id_)
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
if tonumber(admins[i].user_id_) ~= tonumber(BANDE) then
...
DevAbs:del(BANDE..'Abs:Owner:'..msg.chat_id_)
end
end
if Owner(msg) then
if txts[2] == 'المنشئين الاساسيين' or txtss[2] == 'المنشئين الاساسيين' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم حذف المنشئين الاساسيين")  
DevAbs:del(BANDE..'Abs:BasicConstructor:'..msg.chat_id_)
end
end
if BasicConstructor(msg) then
if txts[2] == 'المنشئين' or txtss[2] == 'المنشئين' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم حذف المنشئين")  
DevAbs:del(BANDE..'Abs:Constructor:'..msg.chat_id_)
end end
if Constructor(msg) then
if txts[2] == 'المدراء' or txtss[2] == 'المدراء' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","✯︙تم حذف المدراء")  
DevAbs:del(BANDE..'Abs:Managers:'..msg.chat_id_)
end 
...
DevAbs:srem(BANDE..'Abs:Tkeed:'..msg.chat_id_, v)
end 
...
DevAbs:del(BANDE..'Abs:Filters:'..msg.chat_id_)
DevAbs:del(BANDE.."Abs:FilterAnimation"..msg.chat_id_)
DevAbs:del(BANDE.."Abs:FilterPhoto"..msg.chat_id_)
DevAbs:del(BANDE.."Abs:FilterSteckr"..msg.chat_id_)
end
...
DevAbs:del(BANDE..'Abs:Ban:'..msg.chat_id_) DevAbs:del(BANDE..'Abs:Admins:'..msg.chat_id_) DevAbs:del(BANDE..'User:Donky:'..msg.chat_id_) DevAbs:del(BANDE..'Abs:VipMem:'..msg.chat_id_) DevAbs:del(BANDE..'Abs:Filters:'..msg.chat_id_) DevAbs:del(BANDE..'Abs:Muted:'..msg.chat_id_)
...
local basicconstructor = DevAbs:smembers(BANDE..'Abs:BasicConstructor:'..msg.chat_id_)
local constructor = DevAbs:smembers(BANDE..'Abs:Constructor:'..msg.chat_id_)
local Managers = DevAbs:smembers(BANDE..'Abs:Managers:'..msg.chat_id_)
local admins = DevAbs:smembers(BANDE..'Abs:Admins:'..msg.chat_id_)
local vipmem = DevAbs:smembers(BANDE..'Abs:VipMem:'..msg.chat_id_)
local donky = DevAbs:smembers(BANDE..'User:Donky:'..msg.chat_id_)
...
spam_c = DevAbs:get(BANDE..'Abs:Spam:Text'..msg.chat_id_)
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_, "Spam:User") == "kick" then     
flood = "بالطرد"     
elseif DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "keed" then     
flood = "بالتقيد"     
elseif DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "mute" then     
flood = "بالكتم"           
elseif DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "del" then     
flood = "بالحذف"
else     
flood = "مفتوح"     
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE.."Abs:Lock:Bots"..msg.chat_id_) == "del" then
lock_bots = "بالحذف"
elseif DevAbs:get(BANDE.."Abs:Lock:Bots"..msg.chat_id_) == "ked" then
lock_bots = "بالتقيد"   
elseif DevAbs:get(BANDE.."Abs:Lock:Bots"..msg.chat_id_) == "kick" then
lock_bots = "بالطرد"    
else
lock_bots = "مفتوحه"    
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE..'Abs:Lock:Text'..msg.chat_id_) then mute_text = 'مقفله' else mute_text = 'مفتوحه'end
if DevAbs:get(BANDE..'Abs:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفله' else mute_photo = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفله' else mute_video = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفله' else mute_gifs = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Music'..msg.chat_id_) then mute_music = 'مقفله' else mute_music = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفله' else mute_in = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفله' else mute_voice = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفله' else mute_edit = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Links'..msg.chat_id_) then mute_links = 'مقفله' else mute_links = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفله' else lock_pin = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفله' else lock_sticker = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفله' else lock_tgservice = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفله' else lock_wp = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفله' else lock_htag = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفله' else lock_tag = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Location'..msg.chat_id_) then lock_location = 'مقفله' else lock_location = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفله' else lock_contact = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:English'..msg.chat_id_) then lock_english = 'مقفله' else lock_english = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفله' else lock_arabic = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفله' else lock_forward = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Document'..msg.chat_id_) then lock_file = 'مقفله' else lock_file = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفله' else markdown = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفله' else lock_spam = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفل' else lock_Join = 'مفتوح' end
if DevAbs:get(BANDE.."Abs:Lock:Welcome"..msg.chat_id_) then send_welcome = 'مقفله' else send_welcome = 'مفتوحه' end
if DevAbs:get(BANDE..'Abs:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل' end
if DevAbs:get(BANDE..'Abs:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل' end
if DevAbs:get(BANDE..'Abs:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحه' else lock_taf = 'مقفله' end
if DevAbs:get(BANDE..'Abs:Lock:Farsi'..msg.chat_id_) then lock_farsi = 'مقفله' else lock_farsi = 'مفتوحه' end
local Flood_Num = DevAbs:hget(BANDE.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل كول'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Speak:after:me'..msg.chat_id_) 
end
if text == "تعطيل كول" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل كول'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Speak:after:me'..msg.chat_id_,true)  
end
if text and text:match("^كول (.*)$") and ChCheck(msg) and not DevAbs:get(BANDE.."Speak:after:me"..msg.chat_id_) then
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل ميزة انطق'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Antk:Abs'..msg.chat_id_) 
end
if text == "تعطيل انطق" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل ميزة انطق'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Antk:Abs'..msg.chat_id_,true)  
end
if text and text:match("^انطق (.*)$") and not DevAbs:get(BANDE..'Abs:Antk:Abs'..msg.chat_id_) and ChCheck(msg) then
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء حفظ قوانين المجموعه', 1, 'md')
DevAbs:del(BANDE..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(BANDE..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:set(BANDE..'Abs:rules'..msg.chat_id_,text)
...
DevAbs:set(BANDE..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
end
if text and text:match("^القوانين$") and ChCheck(msg) then
local rules = DevAbs:get(BANDE..'Abs:rules'..msg.chat_id_)
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Zrf:Abs'..msg.chat_id_) 
end
if text == "تعطيل الزخرفه" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الزخرفه بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Zrf:Abs'..msg.chat_id_,true)  
end
if DevAbs:get(BANDE..'Zrf:Abs'..msg.chat_id_..msg.sender_user_id_) then 
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر الزخرفه', 1, 'md')
DevAbs:del(BANDE..'Zrf:Abs'..msg.chat_id_..msg.sender_user_id_)
return false  
end 
...
if not DevAbs:get(BANDE..'Abs:Zrf:Abs'..msg.chat_id_) then
if text == 'زخرفه' and ChCheck(msg) or text == 'الزخرفه' and ChCheck(msg) then  
DevAbs:setex(BANDE.."Zrf:Abs"..msg.chat_id_..msg.sender_user_id_,300,true)
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Brg:Abs'..msg.chat_id_) 
end
if text == "تعطيل الابراج" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الابراج بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Brg:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(BANDE..'Abs:Brg:Abs'..msg.chat_id_) then
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Nsba:Abs'..msg.chat_id_) 
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Nsba:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(BANDE..'Abs:Nsba:Abs'..msg.chat_id_) then
if text == "نسبه الحب" and ChCheck(msg) or text == "نسبة الحب" and ChCheck(msg) then
DevAbs:set(BANDE..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙قم بارسل اسمين لحساب نسبة الحب بينهما كمثال ↫ جاك و روز', 1, 'md')
end
end
if text and text ~= "نسبه الحب" and text ~= "نسبة الحب" and DevAbs:get(BANDE..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر نسبة الحب ', 1, 'md')
DevAbs:del(BANDE..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
...
DevAbs:set(BANDE..'RyNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true)
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙قم بارسل اسمين لحساب نسبة الخيانه بينهما كمثال ↫ جاك و روز', 1, 'md')
end
end
if text and text ~= "نسبه الخيانه" and text ~= "نسبة الخيانه" and text ~= "↫ نسبه الخيانه ✯" and DevAbs:get(BANDE..'RyNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر نسبة الخيانه ', 1, 'md')
DevAbs:del(BANDE..'RyNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(BANDE..'JNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙قم بارسل اسم الشخص لقياس نسبة جماله كمثال ↫ جاك او روز', 1, 'md')
end
end
if text and text ~= "نسبه الجمال" and text ~= "نسبة الجمال" and text ~= "↫ نسبه الجمال ✯" and DevAbs:get(BANDE..'JNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر نسبة الجمال ', 1, 'md')
DevAbs:del(BANDE..'JNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(BANDE..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙قم بارسل اسمين لحساب نسبة الكره بينهما كمثال ↫ جاك و روز', 1, 'md')
end
end
if text and text ~= "نسبه الكره" and text ~= "نسبة الكره" and text ~= "↫ نسبه الكره ✯" and DevAbs:get(BANDE..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر نسبة الكره ', 1, 'md')
DevAbs:del(BANDE..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(BANDE..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙قم بارسل اسم الشخص لقياس نسبة رجولته كمثال ↫ جاك', 1, 'md')
end
end
if text and text ~= "نسبه الرجوله" and text ~= "نسبة الرجوله" and text ~= "نسبه رجوله" and text ~= "نسبة رجوله" and text ~= "↫ نسبه الرجوله ✯" and DevAbs:get(BANDE..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر نسبة الرجوله ', 1, 'md')
DevAbs:del(BANDE..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(BANDE..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙قم بارسل اسم الشخص لقياس نسبة انوثته كمثال ↫ روز', 1, 'md')
end
end
if text and text ~= "نسبه الانوثه" and text ~= "نسبة الانوثه" and text ~= "نسبه انوثه" and text ~= "نسبة انوثه" and text ~= "↫ نسبه الانوثه ✯" and DevAbs:get(BANDE..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر نسبة الانوثه ', 1, 'md')
DevAbs:del(BANDE..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(BANDE..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙قم بارسل اسم الشخص لقياس نسبة غبائه كمثال ↫ جاك او روز', 1, 'md')
end
end
if text and text ~= "نسبه الغباء" and text ~= "نسبة الغباء" and text ~= "↫ نسبه الغباء ✯" and DevAbs:get(BANDE..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '✯︙تم الغاء امر نسبة الغباء ', 1, 'md')
DevAbs:del(BANDE..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل حساب العمر'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Age:Abs'..msg.chat_id_) 
end
if text == "تعطيل حساب العمر" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل حساب العمر'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Age:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(BANDE..'Abs:Age:Abs'..msg.chat_id_) then
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Mean:Abs'..msg.chat_id_) 
end
if text == "تعطيل معاني الاسماء" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل معاني الاسماء'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Mean:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(BANDE..'Abs:Mean:Abs'..msg.chat_id_) then
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل المتحركه'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:gif:Abs'..msg.chat_id_) 
end
if text == "تعطيل متحركه" and Manager(msg) and ChCheck(msg) or text == "تعطيل المتحركه" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل المتحركه'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:gif:Abs'..msg.chat_id_,true)  
end
if text and (text == "متحركه" or text == "↫ متحركه ✯") and not DevAbs:get(BANDE..'Abs:gif:Abs'..msg.chat_id_) and ChCheck(msg) then
Abs = math.random(2,1075); 
local Text ='*✯︙تم اختيار المتحركه لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ✯",callback_data="/gif:"..msg.sender_user_id_}},{{text = '✯ BANDE Team .',url="t.me/Dev-BANDE"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل الميمز'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:memz:Abs'..msg.chat_id_) 
end
if text == "تعطيل ميمز" and Manager(msg) and ChCheck(msg) or text == "تعطيل الميمز" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الميمز'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:memz:Abs'..msg.chat_id_,true)  
end
if text and (text == "ميمز" or text == "↫ ميمز ✯") and not DevAbs:get(BANDE..'Abs:memz:Abs'..msg.chat_id_) and ChCheck(msg) then
Abs = math.random(2,1201); 
local Text ='*✯︙تم اختيار مقطع الميمز لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ✯",callback_data="/memz:"..msg.sender_user_id_}},{{text = '✯ BANDE Team .',url="t.me/WaTaNTeaM"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل غنيلي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Audios:Abs'..msg.chat_id_) 
end
if text == "تعطيل غنيلي" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل غنيلي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Audios:Abs'..msg.chat_id_,true)  
end
if text and (text == "غنيلي" or text == "↫ غنيلي ✯") and not DevAbs:get(BANDE..'Abs:Audios:Abs'..msg.chat_id_) and ChCheck(msg) then
Abs = math.random(4,2824); 
local Text ='*✯︙تم اختيار المقطع الصوتي لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ✯",callback_data="/Song:"..msg.sender_user_id_}},{{text = '✯ BANDE Team .',url="t.me/WaTaNTeaM"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل الاغاني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:mp3:Abs'..msg.chat_id_) 
end
if text == "تعطيل الاغاني" and Manager(msg) and ChCheck(msg) or text == "تعطيل اغنيه" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الاغاني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:mp3:Abs'..msg.chat_id_,true)  
end
if text and (text == "اغنيه" or text == "↫ اغنيه ✯" or text == "اغاني") and not DevAbs:get(BANDE..'Abs:mp3:Abs'..msg.chat_id_) and ChCheck(msg) then
Abs = math.random(2,1167); 
local Text ='*✯︙تم اختيار الاغنيه لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ✯",callback_data="/mp3:"..msg.sender_user_id_}},{{text = '✯ BANDE Team .',url="t.me/WaTaNTeaM"}}} 
local msg_id = msg.id_/2097152/0.5 
https.request("https://api.telegram.org/bot"..TokenBot..'/sendAudio?chat_id=' .. msg.chat_id_ .. '&audio=https://t.me/BANDEMp3/'..Abs..'&caption=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard)) 
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == "تفعيل ريمكس" and Manager(msg) and ChCheck(msg) or text == "تفعيل الريمكس" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل الريمكس'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Remix:Abs'..msg.chat_id_) 
end
if text == "تعطيل ريمكس" and Manager(msg) and ChCheck(msg) or text == "تعطيل الريمكس" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الريمكس'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Remix:Abs'..msg.chat_id_,true)  
end
if text and (text == "ريمكس" or text == "↫ ريمكس ✯") and not DevAbs:get(BANDE..'Abs:Remix:Abs'..msg.chat_id_) and ChCheck(msg) then
Abs = math.random(2,612); 
local Text ='*✯︙تم اختيار الريمكس لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ✯",callback_data="/remix:"..msg.sender_user_id_}},{{text = '✯ BANDE Team .',url="t.me/WaTaNTeaM"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل الصوره'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Photo:Abs'..msg.chat_id_) 
end
if text == "تعطيل صوره" and Manager(msg) and ChCheck(msg) or text == "تعطيل الصوره" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الصوره'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Photo:Abs'..msg.chat_id_,true)  
end
if text and (text == "صوره" or text == "↫ صوره ✯") and not DevAbs:get(BANDE..'Abs:Photo:Abs'..msg.chat_id_) and ChCheck(msg) then
Abs = math.random(4,1171); 
local Text ='*✯︙تم اختيار الصوره لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ✯",callback_data="/photo:"..msg.sender_user_id_}},{{text = '✯ BANDE Team .',url="t.me/WaTaNTeaM"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل الانمي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Anime:Abs'..msg.chat_id_) 
end
if text == "تعطيل انمي" and Manager(msg) and ChCheck(msg) or text == "تعطيل الانمي" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الانمي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Anime:Abs'..msg.chat_id_,true)  
end
if text and (text == "انمي" or text == "↫ انمي ✯") and not DevAbs:get(BANDE..'Abs:Anime:Abs'..msg.chat_id_) and ChCheck(msg) then
Abs = math.random(3,1002); 
local Text ='*✯︙تم اختيار صورة الانمي لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ✯",callback_data="/anime:"..msg.sender_user_id_}},{{text = '✯ BANDE Team .',url="t.me/WaTaNTeaM"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل الافلام'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Movies:Abs'..msg.chat_id_) 
end
if text == "تعطيل فلم" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل الافلام'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Movies:Abs'..msg.chat_id_,true)  
end
if text and (text == "فلم" or text == "↫ فلم ✯") and not DevAbs:get(BANDE..'Abs:Movies:Abs'..msg.chat_id_) and ChCheck(msg) then
Abs = math.random(45,125); 
local Text ='*✯︙تم اختيار الفلم لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ✯",callback_data="/Movies:"..msg.sender_user_id_}},{{text = '✯ BANDE Team .',url="t.me/WaTaNTeaM"}}} 
local msg_id = msg.id_/2097152/0.5 
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل المسلسلات'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Series:Abs'..msg.chat_id_) 
end
if text == "تعطيل مسلسل" and Manager(msg) and ChCheck(msg) then
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل المسلسلات'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Series:Abs'..msg.chat_id_,true)  
end
if text and (text == "مسلسل" or text == "↫ مسلسل ✯") and not DevAbs:get(BANDE..'Abs:Series:Abs'..msg.chat_id_) and ChCheck(msg) then
Abs = math.random(2,54); 
local Text ='*✯︙تم اختيار المسلسل لك*'
keyboard = {}  
keyboard.inline_keyboard = {{{text=". مره اخرى ✯",callback_data="/series:"..msg.sender_user_id_}},{{text = '✯ BANDE Team .',url="t.me/WaTaNTeaM"}}} 
local msg_id = msg.id_/2097152/0.5 
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if Admin(msg) then
if DevAbs:get(BANDE..'Abs:LockSettings'..msg.chat_id_) then 
if text == "الروابط" then if DevAbs:get(BANDE..'Abs:Lock:Links'..msg.chat_id_) then mute_links = 'مقفله' else mute_links = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الروابط ↫ "..mute_links.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "المعرف" or text == "المعرفات" then if DevAbs:get(BANDE..'Abs:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفوله' else lock_tag = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙المعرف ↫ "..lock_tag.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "المتحركه" or text == "الملصقات المتحركه" then if DevAbs:get(BANDE..'Abs:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفوله' else mute_gifs = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙المتحركه ↫ "..mute_gifs.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الملصقات" then if DevAbs:get(BANDE..'Abs:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفوله' else lock_sticker = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الملصقات ↫ "..lock_sticker.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الصور" then if DevAbs:get(BANDE..'Abs:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفوله' else mute_photo = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الصور ↫ "..mute_photo.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الفيديو" or text == "الفيديوهات" then if DevAbs:get(BANDE..'Abs:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفوله' else mute_video = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الفيديو ↫ "..mute_video.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الاونلاين" then if DevAbs:get(BANDE..'Abs:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفل' else mute_in = 'مفتوح' end local Dev-BANDE = "\n" .."✯︙الاونلاين ↫ "..mute_in.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الدردشه" then if DevAbs:get(BANDE..'Abs:Lock:Text'..msg.chat_id_) then mute_text = 'مقفله' else mute_text = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الدردشه ↫ "..mute_text.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "التوجيه" or text == "اعاده التوجيه" then if DevAbs:get(BANDE..'Abs:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفل' else lock_forward = 'مفتوح' end local Dev-BANDE = "\n" .."✯︙التوجيه ↫ "..lock_forward.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الاغاني" then if DevAbs:get(BANDE..'Abs:Lock:Music'..msg.chat_id_) then mute_music = 'مقفوله' else mute_music = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الاغاني ↫ "..mute_music.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الصوت" or text == "الصوتيات" then if DevAbs:get(BANDE..'Abs:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفول' else mute_voice = 'مفتوح' end local Dev-BANDE = "\n" .."✯︙الصوت ↫ "..mute_voice.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الجهات" or text == "جهات الاتصال" then if DevAbs:get(BANDE..'Abs:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفوله' else lock_contact = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الجهات ↫ "..lock_contact.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الماركداون" then if DevAbs:get(BANDE..'Abs:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفل' else markdown = 'مفتوح' end local Dev-BANDE = "\n" .."✯︙الماركداون ↫ "..markdown.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الهاشتاك" then if DevAbs:get(BANDE..'Abs:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفل' else lock_htag = 'مفتوح' end local Dev-BANDE = "\n" .."✯︙الهاشتاك ↫ "..lock_htag.."\n"Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "التعديل" then if DevAbs:get(BANDE..'Abs:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفل' else mute_edit = 'مفتوح' end local Dev-BANDE = "\n" .."✯︙التعديل ↫ "..mute_edit.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "التثبيت" then if DevAbs:get(BANDE..'Abs:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفل' else lock_pin = 'مفتوح' end local Dev-BANDE = "\n" .."✯︙التثبيت ↫ "..lock_pin.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الاشعارات" then if DevAbs:get(BANDE..'Abs:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفوله' else lock_tgservice = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الاشعارات ↫ "..lock_tgservice.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الكلايش" then if DevAbs:get(BANDE..'Abs:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفوله' else lock_spam = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الكلايش ↫ "..lock_spam.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الدخول" then if DevAbs:get(BANDE..'Abs:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفول' else lock_Join = 'مفتوح' end local Dev-BANDE = "\n" .."✯︙الدخول ↫ "..lock_Join.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الشبكات" then if DevAbs:get(BANDE..'Abs:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفوله' else lock_wp = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الشبكات ↫ "..lock_wp.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "المواقع" then if DevAbs:get(BANDE..'Abs:Lock:Location'..msg.chat_id_) then lock_location = 'مقفوله' else lock_location = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙المواقع ↫ "..lock_location.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "العربيه" then if DevAbs:get(BANDE..'Abs:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفوله' else lock_arabic = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙العربيه ↫ "..lock_arabic.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الانكليزيه" then if DevAbs:get(BANDE..'Abs:Lock:English'..msg.chat_id_) then lock_english = 'مقفوله' else lock_english = 'مفتوحه' end local Dev-BANDE = "\n" .."✯︙الانكليزيه ↫ "..lock_english.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الكفر" then if DevAbs:get(BANDE..'Abs:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل' end local Dev-BANDE = "\n" .."✯︙الكفر ↫ "..lock_kaf.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الفشار" then if DevAbs:get(BANDE..'Abs:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل' end local Dev-BANDE = "\n" .."✯︙الفشار ↫ "..lock_fshar.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
if text == "الطائفيه" then if DevAbs:get(BANDE..'Abs:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحه' else lock_taf = 'مقفله' end local Dev-BANDE = "\n" .."✯︙الطائفيه ↫ "..lock_taf.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, Dev-BANDE, 1, 'md') end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == 'تفعيل كشف الاعدادات' and ChCheck(msg) then 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل كشف الاعدادات'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:LockSettings'..msg.chat_id_,true)  
end
if text == 'تعطيل كشف الاعدادات' and ChCheck(msg) then 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل كشف الاعدادات'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:LockSettings'..msg.chat_id_) 
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:Robot'..msg.chat_id_)
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:Robot'..msg.chat_id_,true)
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == 'تفعيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل ردود المدير'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:GpRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل ردود المدير'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:GpRed'..msg.chat_id_,true)
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == 'تفعيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل ردود المطور'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:AllRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل ردود المطور'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:AllRed'..msg.chat_id_,true)
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if AbsSudo(msg) then
...
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if msg.content_.photo_ then
if DevAbs:get(BANDE..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_) then
if msg.content_.photo_.sizes_[3] then
...
DevAbs:del(BANDE..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
return false  end
if data.message_ == "CHAT_ADMIN_REQUIRED" then 
send(msg.chat_id_, msg.id_,"✯︙ليست لدي صلاحية تغير معلومات المجموعه يرجى التحقق من الصلاحيات") 
DevAbs:del(BANDE..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevAbs:set(BANDE..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
if data.ID == "Ok" then
DevAbs:del(BANDE..'Abs:PinnedMsg'..msg.chat_id_)
...
local PinId = DevAbs:get(BANDE..'Abs:PinnedMsg'..msg.chat_id_)
if PinId then
...
DevAbs:del(BANDE..'Abs:Ban:'..msg.chat_id_)
else
local x = 0
...
local Muted = DevAbs:scard(BANDE.."Abs:Muted:"..msg.chat_id_) or "0"
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
tdcli_function({ID="GetChannelFull",channel_id_ = msg.chat_id_:gsub("-100","")},function(arg,data) 
...
local ConstructorList = DevAbs:scard(BANDE.."Abs:Constructor:"..ChatId) or 0
local BanedList = DevAbs:scard(BANDE.."Abs:Ban:"..ChatId) or 0
local ManagerList = DevAbs:scard(BANDE.."Abs:Managers:"..ChatId) or 0
local MutedList = DevAbs:scard(BANDE.."Abs:Muted:"..ChatId) or 0
local TkeedList = DevAbs:scard(BANDE.."Abs:Abs:Tkeed:"..ChatId) or 0
local AdminsList = DevAbs:scard(BANDE.."Abs:Admins:"..ChatId) or 0
local VipList = DevAbs:scard(BANDE.."Abs:VipMem:"..ChatId) or 0
local LinkGp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..ChatId))
if LinkGp.ok == true then LinkGroup = LinkGp.result else LinkGroup = 't.me/Dev-BANDE' end
...
DevAbs:set(BANDE..'Abs:Num:Add:Bot',Num) 
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:FreeBot'..BANDE) 
end 
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE..'Abs:Lock:FreeBot'..BANDE,true) 
end 
end
if ChatType == 'sp' or ChatType == 'gp'  then
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE..'Abs:Lock:Gamesinline'..msg.chat_id_) 
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE.."Abs:Lock:GpLinks"..msg.chat_id_,"ok")
return false  
end
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE.."Abs:Lock:GpLinksinline"..msg.chat_id_,"ok")
return false  
end
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE.."Abs:Lock:GpLinksinline"..msg.chat_id_)
return false  
end
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE.."Abs:Lock:GpRd"..msg.chat_id_)
return false  
end
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE.."Abs:Lock:GpRd"..msg.chat_id_,true)
return false  
end
end
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تفعيل اضف رد'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE.."Abs:Lock:Rd"..msg.chat_id_)
return false  
end
end
...
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل اضف رد'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE.."Abs:Lock:Rd"..msg.chat_id_,true)
return false  
end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
Text = "✯︙تم تفعيل المجموعه "..dp.title_
local inline = {{{text="‹ ترتيب الاوامر ›",callback_data="/SetCmdGp:"..msg.sender_user_id_},{text="‹ رفع الادمنيه ›",callback_data="/UploadAdmin:"..msg.sender_user_id_}},{{text="‹ غادر ›",callback_data="/LeaveBot:"..msg.sender_user_id_},{text="‹ تعطيل ›",callback_data="/Stop:"..msg.sender_user_id_}},{{text="‹ WaTaN TeaM ›",url="t.me/Dev-BANDE"}}}
SendInline(msg.chat_id_,Text,nil,inline,msg.id_/2097152/0.5)
DevAbs:sadd(BANDE.."Abs:Groups",msg.chat_id_)
if not DevAbs:get(BANDE..'Abs:SudosGp'..msg.sender_user_id_..msg.chat_id_) and not SecondSudo(msg) then 
DevAbs:incrby(BANDE..'Abs:Sudos'..msg.sender_user_id_,1)
DevAbs:set(BANDE..'Abs:SudosGp'..msg.sender_user_id_..msg.chat_id_,"Abs")
end
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
if not DevAbs:sismember(BANDE..'Abs:Groups',msg.chat_id_) then
...
local Text = "✯︙Source BANDE\n✯︙File Bot Groups\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local GroupsManagers = DevAbs:scard(BANDE.."Abs:Managers:"..v) or 0
local GroupsAdmins = DevAbs:scard(BANDE.."Abs:Admins:"..v) or 0
local Groupslink = DevAbs:get(BANDE.."Abs:Groups:Links" ..v)
...
DevAbs:setex(BANDE.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n✯︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(BANDE.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(BANDE.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevAbs:smembers(BANDE..'Abs:Users') 
if msg.content_.text_ then
for k,v in pairs(List) do 
AbsText = "الرساله"
...
DevAbs:setex(BANDE.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n✯︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(BANDE.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(BANDE.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevAbs:smembers(BANDE..'Abs:Groups') 
if msg.content_.text_ then
for k,v in pairs(List) do 
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
...
DevAbs:setex(BANDE.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙ارسل الرساله الان لتوجيها \n✯︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(BANDE.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(BANDE.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevAbs:smembers(BANDE..'Abs:Groups')   
for k,v in pairs(List) do  
...
DevAbs:setex(BANDE.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙ارسل الرساله الان لتوجيها \n✯︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(BANDE.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(BANDE.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevAbs:smembers(BANDE..'Abs:Users')   
for k,v in pairs(List) do  
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = msg.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end   
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم اذاعة رسالتك بالتوجيه \n✯︙‏الى ↫ ❨ "..#List.." ❩ مشترك \n ✓", 1, 'md')
...
DevAbs:setex(BANDE.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n✯︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(BANDE.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == "الغاء" then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(BANDE.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
local List = DevAbs:smembers(BANDE.."Abs:Groups") 
if msg.content_.text_ then
...
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
DevAbs:set(BANDE..'Abs:PinnedMsgs'..v,msg.content_.text_) 
end
elseif msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
AbsText = "الصوره"
sendPhoto(v, 0, 0, 1, nil, photo,(msg.content_.caption_ or ''))
DevAbs:set(BANDE..'Abs:PinnedMsgs'..v,photo) 
end 
elseif msg.content_.animation_ then
for k,v in pairs(List) do 
AbsText = "المتحركه"
sendDocument(v, 0, 0, 1,nil, msg.content_.animation_.animation_.persistent_id_,(msg.content_.caption_ or ''))    
DevAbs:set(BANDE..'Abs:PinnedMsgs'..v,msg.content_.animation_.animation_.persistent_id_)
end 
elseif msg.content_.video_ then
for k,v in pairs(List) do 
AbsText = "الفيديو"
sendVideo(v, 0, 0, 1, nil, msg.content_.video_.video_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(BANDE..'Abs:PinnedMsgs'..v,msg.content_.video_.video_.persistent_id_)
end 
elseif msg.content_.voice_ then
for k,v in pairs(List) do 
AbsText = "البصمه"
sendVoice(v, 0, 0, 1, nil, msg.content_.voice_.voice_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(BANDE..'Abs:PinnedMsgs'..v,msg.content_.voice_.voice_.persistent_id_)
end 
elseif msg.content_.audio_ then
for k,v in pairs(List) do 
AbsText = "الصوت"
sendAudio(v, 0, 0, 1, nil, msg.content_.audio_.audio_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(BANDE..'Abs:PinnedMsgs'..v,msg.content_.audio_.audio_.persistent_id_)
end 
elseif msg.content_.document_ then
for k,v in pairs(List) do 
AbsText = "الملف"
sendDocument(v, 0, 0, 1,nil, msg.content_.document_.document_.persistent_id_,(msg.content_.caption_ or ''))    
DevAbs:set(BANDE..'Abs:PinnedMsgs'..v,msg.content_.document_.document_.persistent_id_)
end 
elseif msg.content_.sticker_ then
for k,v in pairs(List) do 
AbsText = "الملصق"
sendSticker(v, 0, 0, 1, nil, msg.content_.sticker_.sticker_.persistent_id_)   
DevAbs:set(BANDE..'Abs:PinnedMsgs'..v,msg.content_.sticker_.sticker_.persistent_id_) 
end 
end
...
if not Bot(msg) and DevAbs:get(BANDE..'Abs:Lock:Rd'..msg.chat_id_) then 
...
local List = DevAbs:smembers(BANDE..'Abs:Manager:GpRedod'..msg.chat_id_)
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙لا توجد ردود متعدده مضافه" ,  1, "md")
return false
end end
DevAbs:set(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙حسنا ارسل كلمة الرد اولا" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local DelGpRedRedod = DevAbs:get(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
if not DevAbs:sismember(BANDE..'Abs:Manager:GpRedod'..msg.chat_id_,text) then
...
DevAbs:set(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedods')
DevAbs:set(BANDE..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
return false
end end
...
DevAbs:set(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedod')
...
DevAbs:set(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SetGpRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙حسنا ارسل الكلمه الان" ,  1, "md")
return false
end end
if text and text:match("^(.*)$") then
local SetGpRedod = DevAbs:get(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
DevAbs:set(BANDE..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SaveGpRedod')
DevAbs:set(BANDE..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
DevAbs:sadd(BANDE..'Abs:Manager:GpRedod'..msg.chat_id_,text)
return false
end end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if text == 'اضف كت تويت' and ChCheck(msg) or text == 'اضف كت' and ChCheck(msg) then
DevAbs:set(BANDE..'Abs:Add:Kt'..msg.sender_user_id_..msg.chat_id_,'SetKt')
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙حسنا ارسل امر `كت تويت` الان" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetKt = DevAbs:get(BANDE..'Abs:Add:Kt'..msg.sender_user_id_..msg.chat_id_)
if SetKt == 'SetKt' then
if text == "الغاء" then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء الامر" ,  1, "md")
DevAbs:del(BANDE..'Abs:Add:Kt'..msg.sender_user_id_..msg.chat_id_)
...
DevAbs:set(BANDE..'Abs:Add:Kt'..msg.sender_user_id_..msg.chat_id_,'SaveKt')
DevAbs:set(BANDE..'Abs:Add:KtTexts'..msg.sender_user_id_..msg.chat_id_,text)
DevAbs:sadd(BANDE..'Abs:Sudo:Kt'..msg.chat_id_,text)
return false
end end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
DevAbs:set(BANDE..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'DelGpRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
...
DevAbs:set(BANDE..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SetGpRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end end
if text and text:match("^(.*)$") then
local SetGpRed = DevAbs:get(BANDE..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
...
DevAbs:set(BANDE..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SaveGpRed')
DevAbs:set(BANDE..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_,text)
DevAbs:sadd(BANDE..'Abs:Manager:GpRed'..msg.chat_id_,text)
DevAbs:set(BANDE..'DelManagerRep'..msg.chat_id_,text)
keyboard = {} 
...
DevAbs:set(BANDE.."Abs:Add:AllRed"..msg.sender_user_id_,'DelAllRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
...
DevAbs:set(BANDE.."Abs:Add:AllRed"..msg.sender_user_id_,'SetAllRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end end
if text and text:match("^(.*)$") then
local SetAllRed = DevAbs:get(BANDE.."Abs:Add:AllRed"..msg.sender_user_id_)
...
Text = "✯︙ارسل لي الرد سواء كان ↫ ⤈\n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n✯︙يمكنك اضافة الى النص ↫ ⤈\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n `#username` ↬ معرف المستخدم\n `#msgs` ↬ عدد الرسائل\n `#name` ↬ اسم المستخدم\n `#id` ↬ ايدي المستخدم\n `#stast` ↬ رتبة المستخدم\n `#edit` ↬ عدد السحكات\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n✯︙للخروج ارسل ↫ ( الغاء )\n ✓"
DevAbs:set(BANDE.."Abs:Add:AllRed"..msg.sender_user_id_,'SaveAllRed')
DevAbs:set(BANDE.."Abs:Add:AllText"..msg.sender_user_id_, text)
DevAbs:sadd(BANDE.."Abs:Sudo:AllRed",text)
DevAbs:set(BANDE.."DelSudoRep",text)
keyboard = {} 
...
local redod = DevAbs:smembers(BANDE..'Abs:Manager:GpRedod'..msg.chat_id_)
MsgRep = '✯︙قائمة الردود المتعدده ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n'
for k,v in pairs(redod) do
MsgRep = MsgRep..k..'~ (`'..v..'`) • {*العدد ↫ '..#DevAbs:smembers(BANDE..'Abs:Text:GpTexts'..v..msg.chat_id_)..'*}\n' 
end
if #redod == 0 then
...
if not Bot(msg) and DevAbs:get(BANDE..'Abs:Lock:GpRd'..msg.chat_id_) then 
...
for k,v in pairs(redod) do
DevAbs:del(BANDE..'Abs:Text:GpTexts'..v..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Manager:GpRedod'..msg.chat_id_)
end
...
local redod = DevAbs:smembers(BANDE..'Abs:Manager:GpRed'..msg.chat_id_)
MsgRep = '✯︙ردود المدير ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n'
for k,v in pairs(redod) do
if DevAbs:get(BANDE.."Abs:Gif:GpRed"..v..msg.chat_id_) then
dp = 'متحركه 🎭'
elseif DevAbs:get(BANDE.."Abs:Voice:GpRed"..v..msg.chat_id_) then
dp = 'بصمه 🎙'
elseif DevAbs:get(BANDE.."Abs:Stecker:GpRed"..v..msg.chat_id_) then
dp = 'ملصق 🃏'
elseif DevAbs:get(BANDE.."Abs:Text:GpRed"..v..msg.chat_id_) then
dp = 'رساله ✉'
elseif DevAbs:get(BANDE.."Abs:Photo:GpRed"..v..msg.chat_id_) then
dp = 'صوره 🎇'
elseif DevAbs:get(BANDE.."Abs:Video:GpRed"..v..msg.chat_id_) then
dp = 'فيديو 📽'
elseif DevAbs:get(BANDE.."Abs:File:GpRed"..v..msg.chat_id_) then
dp = 'ملف 📁'
elseif DevAbs:get(BANDE.."Abs:Audio:GpRed"..v..msg.chat_id_) then
dp = 'اغنيه 🎶'
end
...
for k,v in pairs(redod) do
DevAbs:del(BANDE..'Abs:Gif:GpRed'..v..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Voice:GpRed'..v..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Audio:GpRed'..v..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Photo:GpRed'..v..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Stecker:GpRed'..v..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Video:GpRed'..v..msg.chat_id_)
DevAbs:del(BANDE..'Abs:File:GpRed'..v..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Text:GpRed'..v..msg.chat_id_)
DevAbs:del(BANDE..'Abs:Manager:GpRed'..msg.chat_id_)
end
...
MsgRep = '✯︙ردود المطور ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n'
for k,v in pairs(redod) do
if DevAbs:get(BANDE.."Abs:Gif:AllRed"..v) then
dp = 'متحركه 🎭'
elseif DevAbs:get(BANDE.."Abs:Voice:AllRed"..v) then
dp = 'بصمه 🎙'
elseif DevAbs:get(BANDE.."Abs:Stecker:AllRed"..v) then
dp = 'ملصق 🃏'
elseif DevAbs:get(BANDE.."Abs:Text:AllRed"..v) then
dp = 'رساله ✉'
elseif DevAbs:get(BANDE.."Abs:Photo:AllRed"..v) then
dp = 'صوره 🎇'
elseif DevAbs:get(BANDE.."Abs:Video:AllRed"..v) then
dp = 'فيديو 📽'
elseif DevAbs:get(BANDE.."Abs:File:AllRed"..v) then
dp = 'ملف 📁'
elseif DevAbs:get(BANDE.."Abs:Audio:AllRed"..v) then
dp = 'اغنيه 🎶'
end
MsgRep = MsgRep..k..'~ (`'..v..'`) ↫ {*'..dp..'*}\n' 
end
...
DevAbs:set(BANDE..'Abs:NameBot'..msg.sender_user_id_, 'msg')
return false 
end
end
...
DevAbs:set(BANDE..'Abs:Help0'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local Dev-BANDE =  DevAbs:get(BANDE..'Abs:Help0'..msg.sender_user_id_)
if Dev-BANDE == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(BANDE..'Abs:Help0'..msg.sender_user_id_)
DevAbs:set(BANDE..'Abs:Help', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
✯︙م6 ↫ اوامر الاعضاء
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
✯︙[Source Channel](https://t.me/Dev-BANDE)
]] 
keyboard = {} 
...
DevAbs:set(BANDE..'Abs:Help01'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local Dev-BANDE =  DevAbs:get(BANDE..'Abs:Help01'..msg.sender_user_id_)
if Dev-BANDE == 'msg' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(BANDE..'Abs:Help01'..msg.sender_user_id_)
DevAbs:set(BANDE..'Abs:Help1', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(BANDE..'Abs:Help21'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local Dev-BANDE =  DevAbs:get(BANDE..'Abs:Help21'..msg.sender_user_id_)
if Dev-BANDE == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(BANDE..'Abs:Help21'..msg.sender_user_id_)
DevAbs:set(BANDE..'Abs:Help2', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(BANDE..'Abs:Help31'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local Dev-BANDE =  DevAbs:get(BANDE..'Abs:Help31'..msg.sender_user_id_)
if Dev-BANDE == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(BANDE..'Abs:Help31'..msg.sender_user_id_)
DevAbs:set(BANDE..'Abs:Help3', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(BANDE..'Abs:Help41'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local Dev-BANDE =  DevAbs:get(BANDE..'Abs:Help41'..msg.sender_user_id_)
if Dev-BANDE == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(BANDE..'Abs:Help41'..msg.sender_user_id_)
DevAbs:set(BANDE..'Abs:Help4', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
DevAbs:set(BANDE..'Abs:Help51'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local Dev-BANDE =  DevAbs:get(BANDE..'Abs:Help51'..msg.sender_user_id_)
if Dev-BANDE == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(BANDE..'Abs:Help51'..msg.sender_user_id_)
DevAbs:set(BANDE..'Abs:Help5', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(BANDE..'Abs:Help61'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local Dev-BANDE =  DevAbs:get(BANDE..'Abs:Help61'..msg.sender_user_id_)
if Dev-BANDE == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(BANDE..'Abs:Help61'..msg.sender_user_id_)
DevAbs:set(BANDE..'Abs:Help6', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
local Leave = DevAbs:get(BANDE..'Abs:Leave')
local Text = [[
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:del(BANDE.."Abs:WhyTube"..msg.chat_id_) 
return false  
end
if text == 'تعطيل اليوتيوب' and Manager(msg) then  
local Dev-BANDE = '✯︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n✯︙تم تعطيل اليوتيوب بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, Dev-BANDE, 14, string.len(msg.sender_user_id_))
DevAbs:set(BANDE.."Abs:WhyTube"..msg.chat_id_,true) 
return false  
end 
if text and text:match('^بحث (.*)$') and not DevAbs:get(BANDE.."Abs:WhyTube"..msg.chat_id_) and ChCheck(msg) then            
...
local Users = DevAbs:smembers(BANDE.."User_Bot")
local Groups = DevAbs:smembers(BANDE..'Chek:Groups')
local Sudos = DevAbs:smembers(BANDE.."Sudo:User")
if DevAbs:get(BANDE..'Name:Bot') then
DevAbs:set(BANDE..'Abs:NameBot',(DevAbs:get(BANDE..'Name:Bot') or 'وطن'))
end
for i = 1, #Users do
local id = Users[i]
if id:match("^(%d+)") then
DevAbs:sadd(BANDE..'Abs:Users',Users[i]) 
end
end
for i = 1, #Sudos do
DevAbs:sadd(BANDE..'Abs:SudoBot:',Sudos[i]) 
end
for i = 1, #Groups do
DevAbs:sadd(BANDE..'Abs:Groups',Groups[i]) 
if DevAbs:get(BANDE.."Private:Group:Link"..Groups[i]) then
DevAbs:set(BANDE.."Abs:Groups:Links"..Groups[i],DevAbs:get(BANDE.."Private:Group:Link"..Groups[i]))
end
if DevAbs:get(BANDE.."Get:Welcome:Group"..Groups[i]) then
DevAbs:set(BANDE..'Abs:Groups:Welcomes'..Groups[i],DevAbs:get(BANDE.."Get:Welcome:Group"..Groups[i]))
end
local list2 = DevAbs:smembers(BANDE..'Constructor'..Groups[i])
for k,v in pairs(list2) do
DevAbs:sadd(BANDE.."Abs:Constructor:"..Groups[i], v)
end
local list3 = DevAbs:smembers(BANDE..'BasicConstructor'..Groups[i])
for k,v in pairs(list3) do
DevAbs:sadd(BANDE.."Abs:BasicConstructor:"..Groups[i], v)
end
local list4 = DevAbs:smembers(BANDE..'Manager'..Groups[i])
for k,v in pairs(list4) do
DevAbs:sadd(BANDE.."Abs:Managers:"..Groups[i], v)
end
local list5 = DevAbs:smembers(BANDE..'Mod:User'..Groups[i])
for k,v in pairs(list5) do
DevAbs:sadd(BANDE.."Abs:Admins:"..Groups[i], v)
end
local list6 = DevAbs:smembers(BANDE..'Special:User'..Groups[i])
for k,v in pairs(list6) do
DevAbs:sadd(BANDE.."Abs:VipMem:"..Groups[i], v)
end
DevAbs:set(BANDE.."Abs:Lock:Bots"..Groups[i],"del") DevAbs:hset(BANDE.."Abs:Spam:Group:User"..Groups[i] ,"Spam:User","keed") 
...
for i,Lock in pairs(LockList) do
DevAbs:set(BANDE..Lock..Groups[i],true)
end
end
...
DevAbs:del(BANDE..'Abs:Text:BotWelcome')
DevAbs:del(BANDE..'Abs:Photo:BotWelcome')
return false
end 
...
DevAbs:del(BANDE..'Abs:Lock:BotWelcome')
return false
end 
...
DevAbs:set(BANDE..'Abs:Lock:BotWelcome',true)
return false
end 
...
DevAbs:del(BANDE..'Abs:Text:BotWelcome')
DevAbs:del(BANDE..'Abs:Photo:BotWelcome')
DevAbs:set(BANDE.."Abs:Set:BotWelcome"..msg.sender_user_id_,"Text") 
return false
end 
if text and DevAbs:get(BANDE.."Abs:Set:BotWelcome"..msg.sender_user_id_) == 'Text' then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء الامر", 1, "md") 
DevAbs:del(BANDE.."Abs:Set:BotWelcome"..msg.sender_user_id_)   
return false
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم حفظ النص ارسل لي صورة الترحيب\n✯︙ارسل ↫ الغاء لحفظ النص فقط", 1, 'md')   
DevAbs:set(BANDE.."Abs:Text:BotWelcome",text) 
DevAbs:set(BANDE.."Abs:Set:BotWelcome"..msg.sender_user_id_,"Photo") 
return false 
end 
if DevAbs:get(BANDE.."Abs:Set:BotWelcome"..msg.sender_user_id_) == 'Photo' then 
if text and text:match("^الغاء$") then 
...
DevAbs:set(BANDE.."Abs:Photo:BotWelcome",msg.content_.photo_.sizes_[1].photo_.persistent_id_)
...
DevAbs:setex(BANDE.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_, 300, true)
end end
...
DevAbs:del(BANDE.."DevText")
end end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
if DevAbs:get(BANDE.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "✯︙تم الغاء الامر", 1, "md") 
DevAbs:del(BANDE.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del(BANDE.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local texxt = string.match(text, "(.*)") 
DevAbs:set(BANDE..'Abs:ChText',texxt)
...
DevAbs:setex(BANDE.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)   
...
local msg_id = msg.id_/2097152/0.5
https.request("https://api.telegram.org/bot"..TokenBot..'/sendPhoto?chat_id=' .. msg.chat_id_ .. '&photo=https://t.me/Dev-BANDE&caption=' .. URL.escape(Text).."&reply_to_message_id="..msg_id.."&parse_mode=markdown&disable_web_page_preview=true&reply_markup="..JSON.encode(keyboard))
return false
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
...
DevAbs:incr(BANDE..'Abs:EditMsg'..result.chat_id_..result.sender_user_id_)
local text = result.content_.text_ or result.content_.caption_
local Text = result.content_.text_
if DevAbs:get(BANDE..'Abs:Lock:EditMsgs'..msg.chat_id_) and not Text and not AbsConstructor(result) then
...
local GetMsgPin = DevAbs:get(BANDE..'Abs:PinnedMsgs'..msg.chat_id_)
if GetMsgPin ~= nil then
if text == GetMsgPin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) if dp.ID == 'Ok' then;DevAbs:del(BANDE..'Abs:PinnedMsgs'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
if GetMsgPin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) DevAbs:del(BANDE..'Abs:PinnedMsgs'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.animation_) then 
...
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
tdcli_function({ID = "ChangeChatMemberStatus",chat_id_=v,user_id_=BANDE,status_={ID = "ChatMemberStatusLeft"},},function(e,g) end, nil) 
DevAbs:srem(BANDE..'Abs:Groups',v)  
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
DevAbs:srem(BANDE..'Abs:Groups',v)  
end
...
DevAbs:sadd(BANDE..'Abs:Groups',v)  
end end,nil) end
end
----- لن ابرئ الذمة لأي شخص يأخذ حرفاً او رقما من ملفي ، ايٍ كان الشخص لن ابرئ الذمة لهُ -----
end 
